...
**最新修复（2025-11-01 22:10）**：

### 修复内容

1. **移除了ModelScope镜像源**（不支持HuggingFace API）
   - 测试发现：ModelScope的API返回格式不兼容HuggingFace
   - 保留镜像源：HF-Mirror（国内高速）、HuggingFace官方
   - 测试验证：HF-Mirror可以正常下载文件
...
3. **验证了下载功能**：
   - ✅ HF-Mirror URL格式正确：`https://hf-mirror.com/{repo_id}/resolve/main/{file_name}`
   - ✅ 可以正确处理307重定向
   - ✅ 可以正常下载文件

### 已知问题

1. **HF-Mirror API返回的文件大小为0**
   - 这是HF-Mirror的API限制，不是代码问题
   - 解决方案：代码已支持"大小未知"模式，会在下载过程中动态获取文件大小
   - 影响：进度条可能不准确，但不影响下载功能

### 待测试

请在UI中测试Qwen3-1.7B模型的下载功能，确认：
1. 下载是否正常启动
2. 进度条是否从0%开始
3. 是否可以正常下载文件
4. 取消功能是否正常工作
