...
**问题**: 智能推荐下载器弹窗的"硬件配置"标签页显示的CPU信息不准确，显示为技术标识符"Intel64 Family 6 Model 191 Stepping 2, GenuineIntel"，而不是友好的品牌名称"13th Gen Intel(R) Core(TM) i7-13700HX"。
...
**位置**: `src/core/hardware_detector.py` 第191-239行

**原理**:
- Windows系统：从注册表`HKEY_LOCAL_MACHINE\HARDWARE\DESCRIPTION\System\CentralProcessor\0`读取CPU品牌
- 检测速度：0.000034秒（极快！）
- 回退机制：Linux/Mac或注册表失败时使用cpuinfo
...
**位置**: `src/core/hardware_detector.py` 第87-96行

**原理**:
- 生成硬件指纹：`{CPU核心数}_{内存GB}_{CPU架构}`
- 每次检测时比较硬件指纹
- 硬件指纹变化时清除缓存，重新检测
...
**位置**: `src/ui/ultrafast_smart_downloader_dialog.py` 第95-145行
