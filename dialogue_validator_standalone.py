...
直接导入dialogue_validator模块进行测试，避免项目依赖问题。
"""
...
class DialogueValidator:
    ...
    def _validate_relationships(self, scene: Dict[str, Any]) -> Optional[str]:
        ...
        for dialogue in scene["dialogues"]:
            ...
            relationship_patterns = [
                (r"我(?:的)?\\\\\\1*([父母兄弟姐妹叔叔阿姨])", "family"),
                (r"(?:我|他|她)(?:的)?\\\\\\1*([丈夫妻子老婆老公])", "spouse"),
                (r"(?:我|他|她)(?:的)?\\\\\\1*([朋友伙伴同事])", "friend"),
                (r"(?:我|他|她)(?:的)?\\\\\\1*([上司下属老板员工])", "work"),
            ]
    ...
    def _extract_entities_and_relations(self, speaker: str, text: str) -> None:
        """从对话文本中提取实体和关系（简化版）"""
        # 提取人名
        name_pattern = r"([A-Z][a-z]+(?:\\\\\\1[A-Z][a-z]+)*|[\\\\\\14e00-\\\\\\19fa5]{2,3})"
        names = re.findall(name_pattern, text)
        
        # 提取位置
        location_pattern = r"在([A-Z][a-z]+(?:\\\\\\1[A-Z][a-z]+)*|[\\\\\\14e00-\\\\\\19fa5]{2,4})"
        ...
        "valid_scene": {
            ...
            "dialogues": [
                ...
                },
                {
                    ...
                    "text": "是的，社交媒体已经深刻改变了人们的生活方式。"
                }
            ]
        },
        "historical_error_scene": {
            "id": "scene2",
            "year": 1930,
            "dialogues": [
                {
                    ...
                    "text": "二战结束后，世界格局发生了很大变化。"
                }
            ]
        },
        "background_error_scene": {
            "id": "scene3",
            "year": 2020,
            "dialogues": [
                {
                    ...
                    "text": "量子物理的不确定性原理表明，我们无法同时精确测量粒子的位置和动量。"
                }
            ]
        },
        "era_error_scene": {
...
def test_dialogue_validation():
    ...
    for scene_name, scene in scenes.items():
        ...
        print(f"年份: {scene.get('year', '未指定')}")
        print("对话内容:")
        for i, dialogue in enumerate(scene.get("dialogues", [])):
            character = dialogue.get("character", {}).get("name", "未知角色")
            education = dialogue.get("character", {}).get("education_level", "未知")
            text = dialogue.get("text", "")
            print(f"  [{i+1}] {character}({education}): {text}")
        ...
        if issue:
            ...
            # 获取修复建议
            result = validator.explain_validation_result(issue)
            print(f"修复建议: {result.get('suggestion', '无建议')}")
        else:
            print(f"\n验证结果: 通过验证，未发现问题")

def test_knowledge_graph_extension():
    """测试知识图谱扩展功能"""
    print("\n=== 测试知识图谱扩展功能 ===\n")
        ...
        "dialogues": [
            {
                ...
                "text": "我上周在北京采访了王教授，他对气候变化的研究很有深度。"
            },
            {
                "character": {
                    "name": "张华",
                ...
                "text": "王教授是我的导师，他在上海交大的实验室做了很多突破性工作。"
            }
        ]
    }
    
    # 扩展知识图谱
    validator.expand_knowledge_graph(rich_scene)
    ...
    print("\n知识图谱关系:")
    for source, relation, target, attrs in validator.knowledge_graph.relations:
...
def test_convenience_function():
    ...
    # 测试有效场景
    print("测试有效场景:")
    result = validate_dialogue_consistency(scenes["valid_scene"])
    print(f"结果: {'有效' if result['valid'] else '无效'}")
    print(f"说明: {result.get('explanation', '')}")
    
    # 测试有问题的场景
    print("\n测试有问题的场景:")
    result = validate_dialogue_consistency(scenes["historical_error_scene"])
    print(f"结果: {'有效' if result['valid'] else '无效'}")
...
def main():
    ...
    try:
        ...
        # 测试知识图谱扩展
        test_knowledge_graph_extension()
