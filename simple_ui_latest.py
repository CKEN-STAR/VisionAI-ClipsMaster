...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                           ...
                           QComboBox, QGroupBox, QRadioButton, QButtonGroup, QProgressDialog, QDialog, QFrame)
...
class LogHandler:
    ...
    def get_logs(self, n=None, level=None, search_text=None, offset=0):
        """
        获取日志记录，支持分页和完整显示

        Args:
            n: 返回的日志数量，None表示返回所有日志
            level: 筛选的日志级别
            search_text: 搜索文本
            offset: 偏移量，用于分页

        Returns:
            dict: 包含日志列表、总数、是否有更多日志的信息
        """
        result = {
            'logs': [],
            'total_count': 0,
        ...
        try:
            ...
            result['filtered_count'] = len(filtered_lines)

            # 应用分页
            if n is None:
                # 返回所有过滤后的日志
                result['logs'] = filtered_lines
                result['has_more'] = False
            else:
                # 分页返回
                start_idx = offset
                end_idx = offset + n
                result['logs'] = filtered_lines[start_idx:end_idx]
                result['has_more'] = end_idx < len(filtered_lines)

        except Exception as e:
            print(f"读取日志失败: {e}")
            # 返回错误信息作为日志内容
            result['logs'] = [f"日志读取错误: {str(e)}\n日志文件路径: {self.log_file}\n"]
            result['total_count'] = 1
            result['filtered_count'] = 1

        return result

    def get_logs_simple(self, n=500, level=None, search_text=None):
        """
        简化版本的get_logs，保持向后兼容性
        ...
        Returns:
            list: 日志记录列表
        """
        result = self.get_logs(n=n, level=level, search_text=search_text)
        return result['logs']
...
class SimpleScreenplayApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 添加标题
        about_title_label = QLabel("VisionAI-ClipsMaster 团队介绍")
        about_title_label.setStyleSheet("font-size: 18px; font-weight: bold; color: #1a5276;")
        about_title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        about_layout.addWidget(about_title_label)
        
        # 添加简介
        about_desc = QTextEdit()
        about_desc.setReadOnly(True)
        about_desc.setMaximumHeight(100)
        about_desc.setStyleSheet("border: none; background-color: transparent;")
        about_desc.setHtml("""
        <div style="text-align: center; margin: 10px;">
            <p style="font-size: 15px;">
                VisionAI-ClipsMaster是利用人工智能创作爆款视频的强大工具
            </p>
        </div>
        """)
        ...
        about_btn = QPushButton("查看团队详情")
        about_btn.setMinimumHeight(40)
        about_btn.setStyleSheet("font-size: 14px; font-weight: bold;")
        ...
        logo_label = QLabel("AI视频创作的未来")
        logo_label.setStyleSheet("font-size: 24px; color: #2c3e50; font-weight: bold;")
...
class LogViewerDialog(QDialog):
    """增强版日志查看器对话框 - 支持完整日志显示和分页"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("系统日志查看器 - 完整版")
        self.setMinimumSize(900, 700)
        self.setWindowFlag(Qt.WindowType.WindowContextHelpButtonHint, False)

        # 分页相关属性
        self.current_offset = 0
        self.page_size = 500  # 默认每页显示500条
        self.total_logs = 0
        self.filtered_logs = 0
        self.current_logs_data = None
    ...
    def init_ui(self):
        ...
        self.search_edit = QLineEdit()  # 改为QLineEdit以提高性能
        ...
        # 每页显示数量选择
        pagesize_layout = QVBoxLayout()
        pagesize_label = QLabel("每页显示:")
        pagesize_label.setFixedHeight(20)
        self.pagesize_combo = QComboBox()
        self.pagesize_combo.addItems(["100", "500", "1000", "2000", "全部"])
        self.pagesize_combo.setCurrentText("500")
        self.pagesize_combo.currentTextChanged.connect(self.on_pagesize_changed)
        self.pagesize_combo.setFixedHeight(28)
        pagesize_layout.addWidget(pagesize_label)
        pagesize_layout.addWidget(self.pagesize_combo)
        control_layout.addLayout(pagesize_layout)
        ...
        control_layout.setStretchFactor(pagesize_layout, 1)
        ...
        self.page_info_label.setStyleSheet("font-weight: bold; color: #2c3e50;")
        pagination_layout.addWidget(self.page_info_label)

        # 下一页按钮
        self.next_btn = QPushButton("下一页 ➡️")
        ...
        # 跳转到顶部/底部按钮
        self.top_btn = QPushButton("⬆️ 顶部")
        self.top_btn.setFixedHeight(30)
        self.top_btn.clicked.connect(self.scroll_to_top)
        pagination_layout.addWidget(self.top_btn)

        self.bottom_btn = QPushButton("⬇️ 底部")
        self.bottom_btn.setFixedHeight(30)
        self.bottom_btn.clicked.connect(self.scroll_to_bottom)
        pagination_layout.addWidget(self.bottom_btn)
        ...
        self.log_text.setFont(QFont("Courier New", 10))  # 稍小字体以显示更多内容
        self.log_text.setLineWrapMode(QTextEdit.LineWrapMode.NoWrap)  # 不自动换行，保持原始格式
        # 设置样式
        ...
        self.range_label.setStyleSheet("color: #6c757d;")
        ...
        self.show_all_btn.setFixedHeight(28)
        self.show_all_btn.clicked.connect(self.show_all_logs)
        ...
        """)
        status_layout.addWidget(self.show_all_btn)
    ...
    def on_pagesize_changed(self, size_text):
        """当每页显示数量改变时"""
        if size_text == "全部":
            self.page_size = None
        else:
    ...
    def prev_page(self):
        ...
        if self.current_offset > 0:
            self.current_offset = max(0, self.current_offset - (self.page_size or 500))
            self.refresh_logs()

    def next_page(self):
        """下一页"""
        if self.page_size and self.current_logs_data and self.current_logs_data.get('has_more', False):
    ...
    def show_all_logs(self):
        """显示所有日志"""
        self.pagesize_combo.setCurrentText("全部")
        self.page_size = None
        self.current_offset = 0
        self.refresh_logs()
    ...
    def refresh_logs(self):
        ...
        try:
            ...
            # 使用新的get_logs方法获取完整日志信息
            self.current_logs_data = log_handler.get_logs(
                n=self.page_size,
                level=level,
            ...
            logs = self.current_logs_data['logs']
            self.total_logs = self.current_logs_data['total_count']
            self.filtered_logs = self.current_logs_data['filtered_count']
            ...
            if not logs:
                ...
                self.log_text.append("没有找到符合条件的日志记录")
                self.update_status_display(0, 0, 0)
                return

            # 显示日志内容
            for i, log in enumerate(logs):
                # 根据日志级别设置颜色
                if "| ERROR" in log or "| CRITICAL" in log or "ERROR:" in log or "CRITICAL:" in log:
                ...
                elif "| DEBUG" in log or "DEBUG:" in log:
                    self.log_text.setTextColor(Qt.GlobalColor.darkGray)
                else:
                    self.log_text.setTextColor(Qt.GlobalColor.black)

                # 显示日志内容（保持原始格式，确保长日志完整显示）
                log_content = log.rstrip()
                # 对于特别长的日志，确保完整显示
                if len(log_content) > 1000:
                    # 分行显示长日志，但保持可读性
                    lines = [log_content[i:i+120] for i in range(0, len(log_content), 120)]
                    for j, line in enumerate(lines):
                        if j == 0:
                            self.log_text.append(line)
                        else:
                            self.log_text.append("    " + line)  # 续行缩进
                else:
                    self.log_text.append(log_content)

            # 更新状态显示
            start_idx = self.current_offset + 1
            end_idx = self.current_offset + len(logs)
            self.update_status_display(start_idx, end_idx, self.filtered_logs)
            ...
            self.log_text.clear()
            self.log_text.setTextColor(Qt.GlobalColor.red)
            self.log_text.append(f"刷新日志时发生错误: {str(e)}")
            self.status_label.setText("日志刷新失败")
            self.range_label.setText("显示范围: 错误")

    def update_status_display(self, start_idx, end_idx, total_filtered):
        """更新状态显示"""
        if total_filtered == 0:
            self.status_label.setText("没有日志记录")
            self.range_label.setText("显示范围: 0/0")
            self.page_info_label.setText("第 0 页")
        else:
            # 状态信息
            status_text = f"总计 {self.total_logs} 条日志"
            if self.filtered_logs != self.total_logs:
                status_text += f"，过滤后 {self.filtered_logs} 条"
            self.status_label.setText(status_text)

            # 显示范围
            self.range_label.setText(f"显示范围: {start_idx}-{end_idx} / {total_filtered}")

            # 页码信息
            if self.page_size:
                current_page = (self.current_offset // self.page_size) + 1
                total_pages = (total_filtered + self.page_size - 1) // self.page_size
                self.page_info_label.setText(f"第 {current_page} / {total_pages} 页")
            else:
                self.page_info_label.setText("显示全部")
    ...
    def update_pagination_buttons(self):
        ...
        # 下一页按钮
        has_more = self.current_logs_data and self.current_logs_data.get('has_more', False)
        self.next_btn.setEnabled(has_more and self.page_size is not None)

        # 显示全部按钮
        is_showing_all = self.page_size is None
        self.show_all_btn.setEnabled(not is_showing_all and self.filtered_logs > 0)
    ...
    def clear_logs(self):
        ...
        reply = QMessageBox.question(
            self,
            "确认清空",
            f"确定要清空所有 {self.total_logs} 条日志记录吗？\n此操作不可恢复。",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            QMessageBox.StandardButton.No
        )
