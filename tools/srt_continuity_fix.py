...
def extract_episode_from_text(text: str) -> str:
    """Extract episode id from '#ORIGINAL:' metadata line.
    Expected format produced by UI: '#ORIGINAL: episode=1, index=5, start=..., end=...'
    Returns normalized zero-padded string like '01' when numeric.
    """
    if '#ORIGINAL:' not in text:
        return ''
    # grab the metadata portion
    try:
        meta = text.split('#ORIGINAL:', 1)[1]
    except Exception:
        return ''
    # parse k=v pairs
    ep_val = None
    for part in meta.split(','):
        ...
        if '=' in part:
            k, v = part.split('=', 1)
            if k.strip().lower() == 'episode':
                ep_val = v.strip()
                break
    if not ep_val:
        # fallback: simple token after '#ORIGINAL:'
        m = re.search(r"#ORIGINAL:\s*([A-Za-z0-9_\-]+)", text)
        ep_val = m.group(1) if m else ''
    if not ep_val:
        return ''
    # normalize: keep digits and letters; zero-pad 2 if purely digits
    if ep_val.isdigit():
        return ep_val.zfill(2)
    # handle forms like 'ep1', 'E01'
    m2 = re.search(r"(\d+)", ep_val)
    if m2:
...
def build_labels_from_filenames(mixed_path: str, source_root: str, items: List[Dict]) -> List[str]:
    # Step 1: use inline metadata if available
    preliminary: List[str] = []
    need_fallback = False
    for it in items:
        ep = extract_episode_from_text(it.get('text',''))
        if ep:
            preliminary.append(ep)
        else:
            preliminary.append('UNKNOWN')
            need_fallback = True
    # Step 2: if some are UNKNOWN and source_root is provided, do fuzzy mapping
    if need_fallback and source_root:
        srt_paths = list_source_srts(source_root, mixed_path)
        sources = build_source_index(srt_paths, source_root) if srt_paths else []
        if sources:
            for i,it in enumerate(items):
                if preliminary[i] == 'UNKNOWN':
                    preliminary[i] = best_match_episode(normalize_text(it.get('text','')), sources)
    return preliminary


def fix_ab_singletons(labels: List[str], items: List[Dict], min_block: int = 2) -> List[int]:
    """Block-level single-pass: detect A B A where len(B)<min_block, move B after second A.
    Works on compressed blocks to avoid index drift.
    """
    n = len(items)
    if n <= 2:
        return list(range(n))
    blocks = compress_blocks(labels)
    # Build new order by iterating blocks and reordering when encountering A B A with short B
    order: List[int] = []
    j = 0
    while j < len(blocks):
        lab1, s1, e1, n1 = blocks[j]
        if j+2 < len(blocks):
            lab2, s2, e2, n2 = blocks[j+1]
            lab3, s3, e3, n3 = blocks[j+2]
            if lab1 == lab3 and lab2 != lab1 and n2 < min_block:
                # append block1, then block3, then block2
                order.extend(range(s1, e1+1))
                order.extend(range(s3, e3+1))
                order.extend(range(s2, e2+1))
                j += 3
                continue
        # default: append current block as-is
        order.extend(range(s1, e1+1))
        j += 1
    return order
...
def main():
    ...
    # If labels are all UNKNOWN and source-root missing, do nothing
