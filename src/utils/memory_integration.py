...
class MemoryManager:
    ...
    def __init__(self, env_profile_name: str = None):
        ...
        # 覆盖 MemoryPressurer 参数自 central memory_config
        try:
            from src.utils.memory_config import get_memory_config
            _cfg = get_memory_config()
            _p = (_cfg.get("pressurer") or {})
            # 阈值优先取 pressurer 段，其次顶层
            self.memory_pressurer.threshold_warning = float(_p.get("threshold_warning", _cfg.get("threshold_warning", 0.75)))
            self.memory_pressurer.threshold_critical = float(_p.get("threshold_critical", _cfg.get("threshold_critical", 0.90)))
            # 监控与安全边际
            if "safety_margin_mb" in _p:
                self.memory_pressurer.safety_margin_mb = int(_p["safety_margin_mb"])
            if "monitor_interval" in _p:
                self.memory_pressurer.monitor_interval = float(_p["monitor_interval"])
            logger.info(
                f"内存配置已应用: warn={self.memory_pressurer.threshold_warning:.2f}, "
                f"crit={self.memory_pressurer.threshold_critical:.2f}, "
                f"margin={self.memory_pressurer.safety_margin_mb}MB, "
                f"interval={self.memory_pressurer.monitor_interval}s")
        except Exception as _e:
            logger.debug(f"加载 memory_config 失败或使用默认值: {_e}")
    ...
    def optimize_memory(self, aggressive: bool = False):
        """
        优化内存使用，返回 (before_used_mb, after_used_mb)
        - aggressive=True 时使用更强的清理策略
        """
        try:
            import gc
            try:
                import psutil
                before_used_mb = psutil.virtual_memory().used / (1024 * 1024)
            except Exception:
                # 回退：通过 memory_guard 估算已用（总-可用）
                mem = self.memory_guard.get_memory_status()
                total_mb = mem.get("total_memory", 0) * 1024
                avail_mb = mem.get("available_memory", 0) * 1024
                before_used_mb = max(total_mb - avail_mb, 0)

            # 释放由压力器占用的内存
            try:
                if hasattr(self, "memory_pressurer") and self.memory_pressurer:
                    self.memory_pressurer.release_all()
            except Exception as e:
                logger.debug(f"释放压力内存失败: {e}")
            ...
            try:
                import psutil
                after_used_mb = psutil.virtual_memory().used / (1024 * 1024)
            except Exception:
                mem2 = self.memory_guard.get_memory_status()
                total_mb2 = mem2.get("total_memory", 0) * 1024
                avail_mb2 = mem2.get("available_memory", 0) * 1024
                after_used_mb = max(total_mb2 - avail_mb2, 0)

            return float(before_used_mb), float(after_used_mb)
        except Exception as e:
            logger.warning(f"optimize_memory 执行失败: {e}")
            # 出错时返回 0 以避免调用侧崩溃
            return 0.0, 0.0
