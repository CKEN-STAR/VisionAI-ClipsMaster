...
async def router_lifespan(app):
    ...
    try:
        ...
        logger.error(f"关闭增量更新广播器失败: {str(e)}")

# 创建路由器
router = APIRouter(tags=["clips"], lifespan=router_lifespan)
...
async def process_clip_task(task_id: str, request: ClipRequest):
    ...
    # 获取生成器实例（测试/默认使用CPU路径，避免导入期触发GPU依赖）
    generator = ClipGenerator(use_gpu=False)
