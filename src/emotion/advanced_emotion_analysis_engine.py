...
class AdvancedEmotionAnalysisEngine:
    ...
    def _build_emotion_rules(self) -> Dict[str, EmotionRule]:
        ...
        rules["formal"] = EmotionRule(
            ...
            patterns=[
                ...
                r"Thank you.*attention",
                r"Ladies and gentlemen"
            ],
            keywords=["皇上", "陛下", "殿下", "臣妾", "奏请", "禀报", "恭敬", "谨慎", "请问", "劳烦",
                     # 英文关键词
                     "Good morning", "Thank you", "Ladies", "gentlemen", "announcement", "meeting"],
            context_enhancers=["宫廷", "朝堂", "正式", "礼仪", "尊敬", "formal", "official", "ceremony"],
            weight=1.2,
        ...
        rules["urgent"] = EmotionRule(
            ...
            keywords=["速速", "立即", "马上", "赶紧", "紧急", "火急", "急迫", "急忙", "迅速",
                     # 英文关键词
                     "urgent", "immediately", "emergency", "quickly", "hurry", "rush"],
            context_enhancers=["时间", "deadline", "催促", "等不及", "time", "urgent", "emergency"],
            weight=1.3,
        ...
        rules["serious"] = EmotionRule(
            ...
            patterns=[
                ...
                r"[不可|不能].*[轻举妄动|掉以轻心]",
                # 英文模式
                r"important.*[decision|announcement]",
                r"serious.*[matter|issue]",
                r"considering.*merger"
            ],
            keywords=["重大", "严重", "重要", "关键", "严肃", "庄重", "慎重", "谨慎",
                     # 英文关键词
                     "important", "serious", "critical", "significant", "major", "considering", "merger"],
            context_enhancers=["决策", "后果", "影响", "责任", "decision", "consequence", "impact", "responsibility"],
            weight=1.1,
            confidence_boost=0.3
        )
        ...
        rules["authoritative"] = EmotionRule(
            ...
            patterns=[
                ...
                r"We.*schedule.*meeting"
            ],
            keywords=["传朕", "旨意", "命令", "决定", "安排", "指示", "要求", "召集",
                     # 英文关键词
                     "board", "approved", "decision", "schedule", "meeting", "return", "immediately"],
            context_enhancers=["权力", "地位", "控制", "领导", "authority", "power", "control", "leadership"],
            weight=1.3,
            confidence_boost=0.4
        )
        ...
        rules["professional"] = EmotionRule(
            ...
            patterns=[
                ...
                r"[我们|公司].*[氛围|环境].*[很好|不错]",
                # 英文模式
                r"Mission Control.*Captain",
                r"transmission.*clear",
                r"Data.*coming.*through"
            ],
            keywords=["公司", "工作", "面试", "职业", "专业", "业务", "企业", "氛围",
                     # 英文关键词
                     "Mission", "Control", "Captain", "transmission", "data", "protocols", "monitoring"],
            context_enhancers=["职场", "商务", "正式", "规范", "professional", "business", "corporate", "technical"],
            weight=1.1,
            confidence_boost=0.3
        )
        ...
        rules["surprised"] = EmotionRule(
            ...
            keywords=["serious", "change", "everything", "unprecedented", "amazing", "incredible"],
            context_enhancers=["surprise", "shock", "unexpected"],
            weight=1.2,
            confidence_boost=0.4
        )

        # 好奇/询问情感（主要针对英文）
        rules["curious"] = EmotionRule(
            emotion="curious",
            patterns=[
                r"Can you describe",
                r"What.*seeing",
            ...
            keywords=["describe", "seeing", "explain", "tell", "more", "details"],
            context_enhancers=["question", "inquiry", "curiosity"],
            weight=1.0,
            confidence_boost=0.3
        )

        # 谨慎/小心情感（主要针对英文）
        rules["cautious"] = EmotionRule(
            emotion="cautious",
            patterns=[
                r"Maintain.*distance",
            ...
            keywords=["maintain", "distance", "monitoring", "careful", "cautious", "safe"],
            context_enhancers=["safety", "caution", "careful"],
            weight=1.1,
            confidence_boost=0.3
        )
    ...
    def _build_sentence_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            "question": [r".*[？|?]$", r"^[什么|哪里|怎么|为什么].*", r".*[吗|呢][？|?]?$",
                        r"^What.*", r"^Can you.*", r"^Are you.*", r"^How.*"],
            "exclamation": [r".*[！|!]$", r"^[太|真|好].*[！|!]$",
                           r".*everything!$", r".*serious!$"],
            "statement": [r".*[。|.]$", r"^[我|他|她|它].*",
                         r"^We.*", r"^This.*", r"^I.*"],
            "command": [r"^[请|让|叫].*", r".*[吧|了]$",
                       r"^Return.*", r"^Maintain.*", r"^Continue.*"],
            "formal_address": [r"^[皇上|陛下|殿下].*", r".*[臣妾|奴才].*",
                              r"^Mission Control.*", r"^Captain.*", r"^Good morning.*"]
        }
    ...
    def _build_negation_patterns(self) -> List[str]:
        ...
        return [
            ...
            r"非[常|要|得]",
            # 英文否定模式
            r"not.*[good|right|correct]",
            r"don't.*[think|believe]",
            r"can't.*[do|see]",
            r"won't.*[work|happen]"
        ]
