...
class VideoQualityEvaluator:
    ...
    def evaluate_video_quality(self, video_path: str, reference_video: Optional[str] = None,
                               threshold: float = 28.0, sample_rate: int = 10) -> Dict[str, Any]:
        """评估视频质量（对外统一接口）

        Args:
            video_path: 输出视频路径（待评估）
            reference_video: 参考视频路径（通常为原视频）；若为空则仅返回占位指标
            threshold: 判定阈值（PSNR）
            sample_rate: 每秒采样帧数

        Returns:
            Dict[str, Any]: 质量指标与是否达标
        """
        try:
            if not reference_video or not os.path.exists(reference_video) or not os.path.exists(video_path):
                # 参考视频缺失或路径无效，返回占位信息但不中断流程
                quality_logger.warning("evaluate_video_quality: 参考视频或目标视频不存在，返回占位指标")
                return {
                    "success": False,
            ...
            # 利用现有片段比较能力做整段评估
            result = self.compare_video_segments(
                reference_video, video_path, start_time=0.0, duration=5.0, threshold=threshold, sample_rate=sample_rate
            )
            if not result.get("success", False):
                return {
                    "success": False,
                    ...
                    "error_rate": 1.0,
                    "threshold": threshold,
                    "error": result.get("error", "compare failed"),
                }
            # 统一达标判定逻辑（与 verify_output_quality 对齐）
            meets = result.get("min_psnr", 0) > 28.0 and result.get("error_rate", 1.0) < 0.001
            return {
                "success": True,
                "meets_standard": meets,
                "avg_psnr": result.get("avg_psnr", 0.0),
                "min_psnr": result.get("min_psnr", 0.0),
                "error_rate": result.get("error_rate", 1.0),
                "threshold": threshold,
            }
        except Exception as e:
            quality_logger.warning(f"evaluate_video_quality 失败: {e}")
            return {
                "success": False,
                "meets_standard": False,
                "avg_psnr": 0.0,
