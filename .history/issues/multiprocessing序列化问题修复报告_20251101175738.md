# Multiprocessing序列化问题修复报告

**修复时间**：2025-11-01  
**问题类型**：Windows multiprocessing pickle序列化错误  
**严重程度**：🔴 严重（阻止下载功能正常工作）
...
在UI中尝试真实下载模型时失败，错误信息为：
```
Can't pickle <function download_worker_process>: it's not the same object as src.core.enhanced_model_downloader.download_worker_process
```

**根本原因**：
1. Windows上`multiprocessing.Process`使用`spawn`模式启动子进程
2. `spawn`模式需要序列化（pickle）所有传递给子进程的对象
3. 模块重新加载机制破坏了pickle的对象身份检查

**修复方案**：
回退到`threading.Thread`方式（与Qwen2.5时期相同），并改进取消机制和模型大小获取逻辑。
