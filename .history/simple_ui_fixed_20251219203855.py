...
class SimpleScreenplayApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        ai_mode_main_layout = QVBoxLayout()
        
        # 第一行：模式选择（云端/本地）
        mode_row_layout = QHBoxLayout()
        mode_label = QLabel("推理模式：")
        self.ai_mode_combo = QComboBox()
        self.ai_mode_combo.addItem("🖥️ 本地模式", "local")
        self.ai_mode_combo.addItem("☁️ 云端模式", "cloud")
        self.ai_mode_combo.setMinimumWidth(120)
        self.ai_mode_combo.currentIndexChanged.connect(self.on_ai_mode_changed)
        mode_row_layout.addWidget(mode_label)
        mode_row_layout.addWidget(self.ai_mode_combo)
        mode_row_layout.addStretch()
        ai_mode_main_layout.addLayout(mode_row_layout)
        
        # ========== 本地模式设置容器 ==========
        self.local_mode_container = QWidget()
        local_mode_layout = QHBoxLayout(self.local_mode_container)
        local_mode_layout.setContentsMargins(0, 5, 0, 0)
        
        # 创建语言选择单选按钮
        lang_label = QLabel("语言模式：")
        ...
        local_mode_layout.addWidget(lang_label)
        local_mode_layout.addWidget(self.lang_auto_radio)
        local_mode_layout.addWidget(self.lang_zh_radio)
        local_mode_layout.addWidget(self.lang_en_radio)
        local_mode_layout.addStretch()
        
        ai_mode_main_layout.addWidget(self.local_mode_container)
        
        # ========== 云端模式设置容器 ==========
        self.cloud_mode_container = QWidget()
        cloud_mode_layout = QHBoxLayout(self.cloud_mode_container)
        cloud_mode_layout.setContentsMargins(0, 5, 0, 0)
        
        # 平台选择
        platform_label = QLabel("平台：")
        self.cloud_platform_combo = QComboBox()
        self.cloud_platform_combo.addItem("硅基流动", "siliconflow")
        self.cloud_platform_combo.addItem("阿里魔搭社区", "dashscope")
        self.cloud_platform_combo.setMinimumWidth(120)
        self.cloud_platform_combo.currentIndexChanged.connect(self.on_cloud_platform_changed)
        
        # 模型选择
        model_label = QLabel("模型：")
        self.cloud_model_combo = QComboBox()
        self.cloud_model_combo.addItem("Qwen3-235B", "qwen3-235b")
        self.cloud_model_combo.addItem("DeepSeek-V3", "deepseek-v3")
        self.cloud_model_combo.setMinimumWidth(130)
        ...
        self.cloud_api_key_input.setMinimumWidth(200)
        
        # 测试连接按钮
        self.test_cloud_btn = QPushButton("🔗 测试")
        self.test_cloud_btn.setMaximumWidth(70)
        ...
        cloud_mode_layout.addWidget(platform_label)
        cloud_mode_layout.addWidget(self.cloud_platform_combo)
        cloud_mode_layout.addWidget(model_label)
        cloud_mode_layout.addWidget(self.cloud_model_combo)
        cloud_mode_layout.addWidget(api_key_label)
        cloud_mode_layout.addWidget(self.cloud_api_key_input)
        cloud_mode_layout.addWidget(self.test_cloud_btn)
        cloud_mode_layout.addStretch()
        
        # 默认隐藏云端模式容器
        self.cloud_mode_container.setVisible(False)
        ai_mode_main_layout.addWidget(self.cloud_mode_container)
        
        ai_mode_group.setLayout(ai_mode_main_layout)
...
"""
        ...
        if self.cloud_mode_enabled:
            ...
            if not self.cloud_api_key_input.text().strip():
                QMessageBox.information(
                    ...
                    "• 阿里魔搭：https://dashscope.console.aliyun.com",
    ...
    def test_cloud_connection(self):
        ...
        try:
            ...
            self.test_cloud_btn.setText("🔗 测试")
