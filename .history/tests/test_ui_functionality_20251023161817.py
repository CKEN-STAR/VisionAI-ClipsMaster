    ...
    try:
        ...
        for category, functions in UI_FUNCTIONS.items():
            ...
            for func_name, func_info in functions.items():
                ...
                # 简化检查：只要函数存在就认为有实现
                # 因为inspect.getsource()在某些情况下无法正确工作
                try:
                    # 检查函数是否可调用
                    if callable(func):
                        print(f"  ✓ {func_name:30s} - 函数可调用")
                        implemented_functions += 1
                    else:
                        print(f"  ⚠ {func_name:30s} - 不可调用")
                        empty_functions.append(func_name)

                except Exception as e:
                    print(f"  ? {func_name:30s} - 无法检查 ({e})")

        print(f"\n总计: {implemented_functions}/{total_functions} 函数可调用")

        if empty_functions:
            print(f"\n不可调用的函数: {', '.join(empty_functions)}")
