...
def test_core_dependencies():
    ...
    # 先测试AI/ML依赖（避免与PyQt6的DLL冲突）
    import warnings
    for module_name, description in ai_ml_deps:
        try:
            # 忽略FutureWarning（如pynvml弃用警告）
            ...
            # ImportError: 模块未安装
            # OSError: DLL加载失败
            print(f"✗ {module_name:20s} - {description} (失败: {str(e)[:80]})")
...
def test_nvidia_ml_py():
    ...
    try:
        ...
        try:
            ...
            nvidia_ml_version = pkg_resources.get_distribution("nvidia-ml-py").version
            print(f"✓ 使用nvidia-ml-py版本: {nvidia_ml_version}")
        except:
            print(f"⚠ 使用已弃用的pynvml包，建议升级到nvidia-ml-py")
...
def test_gptq_fallback():
    """测试GPTQ量化的fallback（已弃用）"""
    print("=" * 60)
    print("测试4: GPTQ量化fallback检查（已弃用）")
    print("=" * 60)
    ...
    try:
        ...
        if HAS_AUTO_GPTQ:
            print("⚠ auto-gptq已安装，但GPTQ量化功能已弃用")
            print("  推荐工作流程：FP16训练 → GGUF推理")
        else:
            print("✓ auto-gptq未安装（符合预期，GPTQ已弃用）")

        # 验证代码有优雅的fallback
        try:
            from src.core.gptq_quantizer import GPTQQuantizer
            print("✓ GPTQQuantizer类可正常导入（带弃用警告）")
        except ImportError as e:
            print(f"✗ GPTQQuantizer导入失败: {e}")
            assert False, "GPTQQuantizer应该有fallback机制"
    except Exception as e:
        print(f"⚠ GPTQ模块导入失败（可能是torch环境问题）: {str(e)[:50]}")
