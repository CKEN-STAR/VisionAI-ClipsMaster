...
- ❌ 只检查`models/qwen/`和`models/mistral/`子目录
- ❌ 不检查智能下载器的下载路径

### 追加修复

#### 修改文件：`simple_ui_fixed.py`
**修改位置**：行3543-3630（共88行）
...
def _check_model_files(self, lang_mode):
    ...
    支持智能下载器下载的路径（models/qwen2.5-*/fp16, models/qwen3-*/base）
    ...
    if models_dir.exists():
        if lang_mode == "zh":
            for item in models_dir.iterdir():
                if item.is_dir() and item.name.startswith(("qwen", "Qwen")):
                    if self._has_large_files(str(item), 100):
                        return True
```
...
- ✅ 中文模型检测结果: 已安装
- ✅ 英文模型检测结果: 未安装（符合预期）
- ✅ 实际文件验证: models/qwen2.5-1.5b/fp16/model.safetensors (2944.44 MB)
...
4. **统一的检测逻辑**：
   - ✅ 主窗口的`check_models()`方法
   - ✅ 训练标签页的`_check_model_files()`方法
   - ✅ 两处逻辑完全一致
