...
def test_gptq_cleanup():
    ...
    for config_file in config_files:
        ...
        # 检查9: hardware_requirements中的quantization应该是FP16
        hw_req = config.get('hardware_requirements', {})
        hw_req_ok = True
        for tier_name, tier_config in hw_req.items():
            if isinstance(tier_config, dict):
                quant = tier_config.get('quantization', '')
                if quant and quant != 'FP16':
                    print(f"❌ hardware_requirements.{tier_name}.quantization = {quant} (应该是FP16)")
                    hw_req_ok = False
                    all_passed = False
        if hw_req_ok:
            print("✅ hardware_requirements中所有quantization都是FP16")
            passed_checks += 1
        total_checks += 1

        # 检查10: benchmark.quantization应该是FP16
        benchmark = config.get('benchmark', {})
        benchmark_quant = benchmark.get('quantization', '')
        if benchmark_quant == 'FP16':
            print("✅ benchmark.quantization = FP16")
            passed_checks += 1
        else:
            print(f"❌ benchmark.quantization = {benchmark_quant} (应该是FP16)")
            all_passed = False
        total_checks += 1
