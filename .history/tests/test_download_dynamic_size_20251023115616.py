...
from src.core.enhanced_model_downloader import ModelDownloadWorker
...
def test_download_file_gets_size_from_server():
    ...
    worker = ModelDownloadWorker("test-model", download_config)
    ...
    mock_get_response.headers = {'content-length': '3221225472'}
    mock_get_response.raise_for_status = Mock()
    mock_get_response.iter_content = Mock(return_value=[b'test' * 100])
    
    with patch.object(worker.session, 'head', return_value=mock_head_response):
        with patch.object(worker.session, 'get', return_value=mock_get_response):
            with patch('builtins.open', create=True):
                with patch('pathlib.Path.exists', return_value=False):
                    with patch('pathlib.Path.stat') as mock_stat:
                        # 模拟文件大小
                        mock_stat.return_value.st_size = 3221225472
                        
                        # 执行下载（会失败因为是模拟，但我们只关心HEAD请求是否被调用）
                        try:
    ...
    # 验证HEAD请求被调用
    worker.session.head.assert_called_once()
    print("✅ 通过：下载文件时正确发送HEAD请求获取文件大小")
...
def test_verify_download_gets_size_from_server():
    ...
    assert result == True
    print("✅ 通过：验证下载时正确发送HEAD请求获取文件大小")
