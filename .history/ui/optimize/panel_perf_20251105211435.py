...
class PanelOptimizer(QObject):
    ...
    def start_monitoring(self, interval_ms: int = 3000) -> None:
        """启动周期性面板优化"""
        try:
            if not hasattr(self, "_monitor_timer") or self._monitor_timer is None:
                self._monitor_timer = QTimer(self)
                self._monitor_timer.timeout.connect(self._monitor_tick)
            self._monitor_timer.start(max(200, int(interval_ms)))
            logger.info(f"[OK] PanelOptimizer监控已启动: interval={interval_ms}ms")
        except Exception as e:
            print(f"[WARN] 启动监控失败: {e}")
    ...
    def stop_monitoring(self) -> None:
        ...
        try:
            if hasattr(self, "_monitor_timer") and self._monitor_timer is not None:
                ...
                logger.info("[OK] PanelOptimizer监控已停止")
        except Exception as e:
    ...
    def _monitor_tick(self):
        """定时执行的轻量优化任务"""
        try:
            for panel_id, rec in list(self.optimization_cache.items()):
                panel = rec.get('panel')
                if isinstance(panel, QWidget):
                    self.optimize_panel_rendering(panel)
                    rec['optimization_count'] = rec.get('optimization_count', 0) + 1
                    rec['last_optimized'] = time.time()
        except Exception as e:
            print(f"[WARN] 监控tick异常: {e}")
...
class ThumbnailGenerator(QThread):
    ...
    def _generate_thumbnail(self) -> bool:
        ...
        try:
            ...
            ffmpeg_cmd = 'ffmpeg'
            try:
                from ui.config.environment import FFMPEG_PATH
                if FFMPEG_PATH:
