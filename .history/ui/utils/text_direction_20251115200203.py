...
def set_application_layout_direction(direction: LayoutDirection):
    ...
    try:
        ...
        if isinstance(direction, str):
            dir_is_rtl = LayoutDirection.is_rtl_language(direction)
            app.setLayoutDirection(Qt.LayoutDirection.RightToLeft if dir_is_rtl else Qt.LayoutDirection.LeftToRight)
            print(f"[OK] 布局方向已设置为: {'rtl' if dir_is_rtl else 'ltr'}")
            return
        ...
        if direction == LayoutDirection.LEFT_TO_RIGHT:
            ...
            try:
                ...
                lang = qloc.language()
                if lang in (QLocale.Language.Arabic, QLocale.Language.Hebrew, QLocale.Language.Persian):
                    app.setLayoutDirection(Qt.LayoutDirection.RightToLeft)
                else:
                    app.setLayoutDirection(Qt.LayoutDirection.LeftToRight)
            except Exception:
        ...
        print(f"[OK] 布局方向已设置为: {direction.value if isinstance(direction, LayoutDirection) else str(direction)}")
