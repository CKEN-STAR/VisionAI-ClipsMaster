...
class UltraFastSmartDownloaderDialog(QDialog):
    ...
    def load_recommendations(self):
        ...
        try:
            ...
            elif recommendation and not recommendation.variant:
                # 设备硬件不足，无法推荐任何变体
                logger.warning("⚠️ 设备硬件不足，无法推荐任何变体")

                # 显示硬件不足的详细信息
                self.update_hardware_insufficient_display(recommendation)

                # 禁用下载按钮
                self.download_btn.setEnabled(False)
                self.status_label.setText("⚠️ 设备硬件不足，无法下载此模型")
            else:
...
<div style='font-size: 14px; line-height: 1.8;'>
    ...
    def update_hardware_insufficient_display(self, recommendation):
        """显示硬件不足的详细信息"""
        # 更新推荐卡片样式为警告样式
        self.recommendation_card.setStyleSheet("""
            QGroupBox {
        ...
        info_text = f"""
<div style='font-size: 14px; line-height: 1.8;'>
    <p style='font-size: 16px; font-weight: bold; color: #ff9800; margin-bottom: 10px;'>
        ⚠️ 设备硬件不足
    </p>
    <p style='margin: 10px 0; color: #d32f2f; font-weight: bold;'>
        当前设备无法运行 {self.model_name} 模型
    </p>
"""

        # 添加推荐理由（包含硬件不足的详细说明）
        if recommendation.reasoning:
            info_text += "<p style='margin: 10px 0;'><b>详细说明:</b></p><ul style='margin: 5px 0; padding-left: 20px;'>"
            for reason in recommendation.reasoning:
                info_text += f"<li style='margin: 3px 0;'>{reason}</li>"
            info_text += "</ul>"

        # 添加部署说明（包含解决方案建议）
        if recommendation.deployment_notes:
            info_text += "<p style='margin: 10px 0;'><b>解决方案:</b></p><ul style='margin: 5px 0; padding-left: 20px;'>"
            for note in recommendation.deployment_notes:
                info_text += f"<li style='margin: 3px 0;'>{note}</li>"
            info_text += "</ul>"

        info_text += """
    <p style='margin: 15px 0; padding: 10px; background-color: #fff9c4; border-left: 4px solid #fbc02d;'>
        <b>💡 提示:</b> 您可以尝试选择更小的模型（如Qwen2.5-0.5B），或升级设备硬件。
    </p>
</div>
        """
