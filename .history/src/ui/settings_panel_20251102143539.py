...
class SettingsPanel(QWidget if PYQT_AVAILABLE else object):
    ...
    def _add_model_management_tab(self):
        ...
        # 说明文字
        info_label = QLabel(
            "💡 训练后的模型默认保存为HuggingFace格式（LoRA适配器）。\n"
            "   如需用于推理，需转换为GGUF格式以提升性能。\n\n"
            "🤖 智能转换（推荐）：自动检测模型类型，一键完成合并和转换\n"
            "🔄 手动转换：适用于已合并的完整HuggingFace模型"
        )
        info_label.setWordWrap(True)
        info_label.setStyleSheet("color: #666; font-size: 12px; padding: 5px; background-color: #f8f9fa; border-radius: 5px;")
        convert_layout.addWidget(info_label, 0, 0, 1, 3)
    ...
    def _smart_conversion(self):
        """智能转换：自动检测模型类型并执行相应的转换流程"""
        from pathlib import Path
        ...
        if not source_path:
            self.convert_status_label.setText("❌ 请填写源模型路径")
            self.convert_status_label.setStyleSheet("color: #dc3545; font-size: 12px; padding: 5px;")
            return

        source_path_obj = Path(source_path)
        if not source_path_obj.exists():
            self.convert_status_label.setText(f"❌ 源模型路径不存在: {source_path}")
            self.convert_status_label.setStyleSheet("color: #dc3545; font-size: 12px; padding: 5px;")
            return

        # 检测模型类型
        has_config = (source_path_obj / "config.json").exists()
        has_adapter_config = (source_path_obj / "adapter_config.json").exists()
        has_adapter_model = (source_path_obj / "adapter_model.safetensors").exists()
        ...
        try:
            self.smart_convert_btn.setEnabled(False)
            self.convert_btn.setEnabled(False)
            ...
            if has_adapter_config and has_adapter_model and not has_config:
                self.convert_status_label.setText("🔍 检测到LoRA适配器，开始智能转换流程...")
                self.convert_status_label.setStyleSheet("color: #007bff; font-size: 12px; padding: 5px;")
                ...
                reply = QMessageBox.question(
                    ...
                    "需要基础模型",
                    "检测到这是一个LoRA适配器目录。\n\n"
                    "智能转换需要将LoRA合并到基础模型后再转换为GGUF。\n\n"
                    "是否使用默认基础模型？\n"
                    "默认路径: models/qwen3-1.7b/fp16",
                    QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
                )

                if reply == QMessageBox.StandardButton.No:
                    self.convert_status_label.setText("❌ 用户取消操作")
                    self.convert_status_label.setStyleSheet("color: #dc3545; font-size: 12px; padding: 5px;")
                    return

                base_model_path = "models/qwen3-1.7b/fp16"

                # 步骤1：合并LoRA
                self.convert_status_label.setText(
                    "📋 步骤1/2：合并LoRA适配器到基础模型\n"
                    "⏳ 正在加载模型和适配器，这可能需要几分钟..."
                )
                self.convert_status_label.setStyleSheet("color: #007bff; font-size: 12px; padding: 5px;")
                QApplication.processEvents()
                ...
                merged_path = converter.merge_lora_to_base(
                    ...
                    lora_adapter_path=source_path,
                    output_path=str(source_path_obj.parent / "merged")
                )
                ...
                self.convert_status_label.setText(
                    f"📋 步骤2/2：转换为GGUF格式\n"
                    f"⏳ 正在转换为 {quant_type} 量化，这可能需要几分钟..."
                )
                self.convert_status_label.setStyleSheet("color: #007bff; font-size: 12px; padding: 5px;")
                QApplication.processEvents()
                ...
                from datetime import datetime
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_path = f"models/qwen/quantized/trained_{timestamp}_{quant_type}.gguf"
                ...
                self.convert_status_label.setText(
                    f"🎉 智能转换完成！\n\n"
                    f"✅ 步骤1：LoRA合并 ({merged_size:.2f} MB)\n"
                    f"✅ 步骤2：GGUF转换 ({gguf_size:.2f} MB)\n\n"
                    f"📦 最终模型: {gguf_path}\n"
                ...
                # 更新输出路径显示
                self.output_gguf_path.setText(gguf_path)

            # 情况2：完整的HuggingFace模型（直接转换）
            elif has_config:
                self.convert_status_label.setText(
                    f"🔍 检测到完整的HuggingFace模型\n"
                    f"⏳ 正在转换为 {quant_type} 量化..."
                )
                self.convert_status_label.setStyleSheet("color: #007bff; font-size: 12px; padding: 5px;")
                QApplication.processEvents()
                ...
                from datetime import datetime
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                output_path = f"models/qwen/quantized/base_{timestamp}_{quant_type}.gguf"
                ...
                self.convert_status_label.setText(
                    ...
                    "请确保目录包含以下文件之一：\n"
                    "• config.json（完整模型）\n"
                    "• adapter_config.json（LoRA适配器）"
                )
                self.convert_status_label.setStyleSheet("color: #dc3545; font-size: 12px; padding: 5px;")

        except Exception as e:
            self.convert_status_label.setText(f"❌ 智能转换失败: {str(e)}")
            self.convert_status_label.setStyleSheet("color: #dc3545; font-size: 12px; padding: 5px;")
            logger.error(f"智能转换失败: {e}")
            import traceback
            traceback.print_exc()
        finally:
            self.smart_convert_btn.setEnabled(True)
            self.convert_btn.setEnabled(True)
    ...
    def _refresh_model_list(self):
        ...
        try:
            ...
            logger.info(f"🔍 开始扫描模型目录: {models_dir.absolute()}")

            if not models_dir.exists():
                logger.warning(f"❌ models目录不存在: {models_dir.absolute()}")
            ...
            logger.info(f"📁 扫描 {models_dir} 目录下的所有子目录...")
            for model_dir in models_dir.iterdir():
                logger.info(f"  🔍 检查: {model_dir.name}")

                if not model_dir.is_dir():
                    logger.info(f"    ⏭️ 跳过（不是目录）: {model_dir.name}")
                    continue

                # 跳过隐藏目录和临时目录
                if model_dir.name.startswith('.') or model_dir.name.startswith('_'):
                    logger.info(f"    ⏭️ 跳过（隐藏/临时目录）: {model_dir.name}")
                    continue

                # 检查是否包含模型文件（递归查找）
                logger.info(f"    🔎 递归扫描: {model_dir.name}")
                ...
                if model_files:
                    ...
                    logger.info(f"    ❌ 未找到模型文件")

            # 填充表格
            logger.info(f"📊 填充表格，共 {len(detected_models)} 个模型")
            ...
            logger.error(f"❌ 刷新模型列表失败: {str(e)}", exc_info=True)
    ...
    def _scan_model_directory(self, model_dir: Path) -> list:
        ...
        logger.info(f"      🔎 开始递归扫描: {model_dir}")

        # 递归查找所有可能包含模型的目录
        all_dirs = [model_dir] + list(model_dir.rglob("*"))
        logger.info(f"      📁 找到 {len(all_dirs)} 个目录（包括子目录）")

        for root_dir in all_dirs:
            if not root_dir.is_dir():
                continue

            logger.info(f"        🔍 检查目录: {root_dir.relative_to(model_dir) if root_dir != model_dir else '(根目录)'}")
            ...
            logger.info(f"          safetensors: {len(has_safetensors)} 个文件")
            logger.info(f"          gguf: {len(has_gguf)} 个文件")
            logger.info(f"          adapter: {has_adapter}")
            ...
            if has_config or has_safetensors or has_gguf or has_adapter:
                ...
                if has_adapter:
                    format_type = "LoRA"
                    description = f"{language}微调模型"
                elif has_safetensors:
                    format_type = "SafeTensors"
                    description = f"{language}基础模型"
                elif has_gguf:
                    format_type = "GGUF"
                    description = f"{language}量化模型"
                else:
                    format_type = "HuggingFace"
                    description = f"{language}基础模型"

                logger.info(f"        📋 模型类型: {model_type}, 格式: {format_type}")

                # 计算大小
                logger.info(f"        📊 计算模型大小...")
                total_size = sum(f.stat().st_size for f in root_dir.rglob("*") if f.is_file())
                size_str = self._format_size(total_size)
                logger.info(f"        💾 模型大小: {size_str}")

                # 获取最后修改时间
                try:
                    mtime = max(f.stat().st_mtime for f in root_dir.rglob("*") if f.is_file())
                    last_modified = datetime.fromtimestamp(mtime).strftime("%Y-%m-%d %H:%M")
                except:
                    last_modified = "未知"

                # 生成显示名称
                display_name = f"{model_type} - {description}"
                if root_dir != model_dir:
                    # 如果是子目录，添加子目录名称
                    subdir_name = root_dir.relative_to(model_dir)
                    display_name += f" ({subdir_name})"

                logger.info(f"        ✅ 添加模型: {display_name}")
                ...
                # 避免重复扫描：如果找到模型，不再扫描子目录
                logger.info(f"        🛑 停止扫描子目录（避免重复）")
                break

        logger.info(f"      ✅ 扫描完成，找到 {len(detected_models)} 个模型")
