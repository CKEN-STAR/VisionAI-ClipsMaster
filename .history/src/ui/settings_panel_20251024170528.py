...
class SettingsPanel(QWidget if PYQT_AVAILABLE else object):
    ...
    def _add_model_management_tab(self):
        ...
        """)
        convert_layout.addWidget(self.convert_btn, 4, 0, 1, 3)
        ...
        # 模型版本管理组
        version_group = QGroupBox("训练版本管理")
        version_layout = QGridLayout(version_group)

        # 版本列表（简化显示）
        version_info_label = QLabel(
            "训练后的模型版本会自动保存在:\n"
            "• 中文模型: models/qwen/trained/\n"
            "• 英文模型: models/mistral/trained/\n\n"
            "每个版本包含HuggingFace格式和GGUF格式（如已转换）"
        )
        version_info_label.setWordWrap(True)
        version_info_label.setStyleSheet("color: #666; font-size: 12px; padding: 5px;")
        version_layout.addWidget(version_info_label, 0, 0)
