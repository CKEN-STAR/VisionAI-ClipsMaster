...
class VideoProcessor(QObject):
    ...
    def generate_viral_srt(srt_file_path, language_mode="auto"):
        """生成爆款SRT文件"""
        try:
            from src.core.screenplay_engineer import import_srt, generate_screenplay, export_srt
            ...
            if language_mode and language_mode != "auto":
                ...
                # 否则自动检测（修复中文检测范围）
                language = "zh" if any('\u4e00' <= ch <= '\u9fff' for ch in text_content) else "en"
            ...
            # 使用内置导出逻辑（优先使用 output_start/output_end，如有）
            export_srt(viral_subtitles, output_path)
            return output_path
    ...
    def generate_video(video_path, srt_path, output_path=None, use_gpu=True):
        """根据视频和SRT生成新视频（调用核心ClipGenerator管线）"""
        try:
            from src.core.clip_generator import ClipGenerator
            ...
            _logger.info(f"[UI] 使用ClipGenerator生成视频: video={video_path}, srt={srt_path}, gpu={use_gpu}")
            cg = ClipGenerator(use_gpu=use_gpu)
            result = cg.generate_from_srt(video_path=video_path, srt_path=srt_path, output_path=output_path)
            if result and str(result.get('status')).lower() == 'success':
                _logger.info(f"[UI] 视频生成成功: {output_path}")
                return output_path
            else:
                err = (result or {}).get('error', '未知错误')
                _logger.error(f"[UI] 视频生成失败: {err}")
                return None
        except Exception as e:
            print(f"[UI] 视频生成异常: {e}")
            return None
...
class SimplifiedTrainingFeeder(QWidget):
    ...
    def check_model_exists(self, lang_mode):
        ...
        zh_model_paths = [
            ...
            os.path.join(Path(__file__).resolve().parent, "models/qwen/base/qwen3-1.7b.bin"),
            os.path.join(Path(__file__).resolve().parent, "models/qwen/base/qwen3-1.7b"),
        ...
        if lang_mode == "zh":
            ...
            if not model_exists:
                # 直接在训练页面弹出确认对话框，而不是调用主窗口的check_en_model
                reply = QMessageBox.question(
                    self,
                    "英文模型未安装",
                    "英文模型尚未下载，是否现在下载？\n(约4GB，需要较长时间)",
...
class SimpleScreenplayApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.init_ui()
        # 移除对setup_params_ui的调用，允许模型自主工作，无需人为干预参数
    ...
    def init_ui(self):
        ...
        # 处理进度条布局
        process_progress_layout = QHBoxLayout()
        self.status_label = QLabel("")
        process_progress_layout.addWidget(self.status_label, 1)

        # 创建并添加进度条
        self.process_progress_bar = QProgressBar()
        self.process_progress_bar.setTextVisible(True)
        process_progress_layout.addWidget(self.process_progress_bar, 3)

        # 将进度条布局添加到主布局
        main_layout.addLayout(process_progress_layout)
        ...
        about_title_label.setStyleSheet("font-size: 28px; font-weight: bold; color: #1a5276; margin-top: 15px;")
        about_title_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        about_layout.addWidget(about_title_label)

        # 添加副标题
        about_subtitle = QLabel("AI驱动的视频创作平台")
        about_subtitle.setStyleSheet("font-size: 16px; color: #2980b9; font-style: italic;")
        about_subtitle.setAlignment(Qt.AlignmentFlag.AlignCenter)
        about_layout.addWidget(about_subtitle)

        # 添加版本信息
        version_label = QLabel("版本 1.0.0-beta | 2025年5月发布")
        ...
        core_group.setStyleSheet("QGroupBox { font-weight: bold; font-size: 15px; }")
        ...
        for feature in core_features:
            ...
            feature_label.setStyleSheet("font-size: 14px; padding: 3px;")
        ...
        tech_group.setStyleSheet("QGroupBox { font-weight: bold; font-size: 15px; }")
        tech_layout = QVBoxLayout(tech_group)

        tech_features = [
            "• 大型语言模型：Mistral-7B, Qwen3系列",
            "• 视频处理：FFmpeg, OpenCV",
            "• 音频处理：Librosa, SoundFile",
            "• NLP处理：SpaCy, SentenceTransformers",
            "• UI界面：PyQt6",
            "• 图像质量评估：SSIM, VIF"
        ]
        ...
        for tech in tech_features:
            ...
            tech_label.setStyleSheet("font-size: 14px; padding: 3px;")
        ...
        team_btn.setMinimumHeight(40)
        team_btn.setMinimumWidth(150)
        team_btn.setStyleSheet("font-size: 14px; font-weight: bold; background-color: #1a5276; color: white;")
        team_btn.clicked.connect(self.show_about_dialog)

        # 技术详情按钮
        tech_btn = QPushButton("技术详情")
        tech_btn.setMinimumHeight(40)
        tech_btn.setMinimumWidth(150)
        tech_btn.setStyleSheet("font-size: 14px; font-weight: bold; background-color: #117a65; color: white;")
        tech_btn.clicked.connect(self.show_tech_dialog)

        # 项目历程按钮
        history_btn = QPushButton("项目历程")
        history_btn.setMinimumHeight(40)
        history_btn.setMinimumWidth(150)
        history_btn.setStyleSheet("font-size: 14px; font-weight: bold; background-color: #7d3c98; color: white;")
        ...
        quote_text.setMaximumHeight(80)
        quote_text.setStyleSheet("border: none; background-color: #f4f6f7; border-radius: 5px; margin: 10px;")
        quote_text.setHtml("""
        <div style="text-align: center; margin: 10px; font-style: italic;">
            <p style="font-size: 15px; color: #34495e;">
                "VisionAI-ClipsMaster旨在彻底改变视频创作方式，让每个人都能轻松制作出专业级的引人入胜的短视频内容。"
            </p>
            <p style="font-size: 13px; color: #7f8c8d; text-align: right;">
                — CKEN-STAR，项目创建者
            </p>
        </div>
        """)
        ...
        # 添加版权信息
        copyright_label = QLabel("© 2025 CKEN-STAR 版权所有")
        copyright_label.setStyleSheet("color: #7f8c8d; margin: 15px 0 5px 0;")
    ...
    def on_tab_changed(self, index):
        ...
        # 在主页面(视频处理页)显示进度条，其他页面隐藏
        if index == 0:  # 视频处理页
            self.process_progress_bar.setVisible(True)
        else:
            self.process_progress_bar.setVisible(False)

        log_handler.log("info", f"切换到标签页: {index}")

    # 修改generate_viral_srt方法添加进度条功能
    def generate_viral_srt(self):
        ...
        self.status_label.setText("爆款SRT生成进度:")
        ...
        for i, item in enumerate(selected_items):
            ...
            # 更新状态和进度条
            self.status_label.setText(f"正在生成爆款SRT ({i+1}/{total_items}): {os.path.basename(srt_path)}")
            self.statusBar().showMessage(f"处理进度: {i+1}/{total_items} - AI模型根据训练数据自主生成，无需人工参数调节")

            # 初始进度为当前项的起始百分比
            current_progress = int((i / total_items) * 100)
            self.process_progress_bar.setValue(current_progress)
            QApplication.processEvents()

            # 模拟生成过程中的进度更新
            for step in range(1, 11):
                time.sleep(0.05)  # 短暂延迟以展示进度
                step_progress = current_progress + int((step / 10) * (100 / total_items))
                self.process_progress_bar.setValue(step_progress)
                QApplication.processEvents()
        ...
        self.status_label.setText(f"成功生成 {total_items} 个爆款SRT")
        self.statusBar().showMessage(f"完成 - AI模型已根据训练数据自主创作，无需人工参数干预")

    # 修改generate_video方法添加进度条功能
    def generate_video(self):
        ...
        # 重置进度条
        self.process_progress_bar.setValue(0)
        self.status_label.setText("视频生成进度:")
        ...
        try:
            ...
            for i, step in enumerate(process_steps):
                # 更新状态
                self.status_label.setText(f"视频生成中: {step}")
            ...
            # 使用实际功能生成视频
            self.process_progress_bar.setValue(85)
            self.status_label.setText("视频生成中: 最终处理...")
            QApplication.processEvents()
            ...
            log_handler.log("error", f"视频处理失败: {str(e)}")
    ...
    def change_language_mode(self, mode):
        ...
        previous_mode = self.language_mode
        ...
        if mode == "en" and not self.en_model_exists:
            # 避免重复弹窗，使用标志位
            if hasattr(self, '_showing_model_dialog') and self._showing_model_dialog:
                return
            ...
            try:
                ...
                if reply == QMessageBox.StandardButton.Yes:
                    ...
                    # 如果用户选择不下载，则回退到上一个语言模式
                    self.language_mode = previous_mode

                    # 恢复对应的单选按钮状态
                    if previous_mode == "auto":
                        self.lang_auto_radio.setChecked(True)
                    elif previous_mode == "zh":
                        self.lang_zh_radio.setChecked(True)

                    log_handler.log("info", f"用户取消下载英文模型，保持当前语言模式: {previous_mode}")
                    return
            finally:
                # 清除弹窗标志
                self._showing_model_dialog = False
        ...
        # 更新UI
        self.status_label.setText(f"已切换到{mode_names[mode]}")
        self.statusBar().showMessage(f"当前使用: {model_info}")
        log_handler.log("info", f"切换语言模式: {mode}, 使用模型: {model_info}")

        # 设置一个标志位，表示这是由主窗口发起的语言切换
    ...
    def open_url(self, url):
        ...
        try:
            import webbrowser
            webbrowser.open(url)
            log_handler.log("info", f"打开URL: {url}")
        except Exception as e:
    ...
    def open_email(self, email):
        ...
        try:
            ...
            log_handler.log("info", f"打开邮件: {email}")
        except Exception as e:
    ...
    def init_ui(self):
        ...
        <div style="text-align: center; margin: 20px;">
            ...
            <p style="font-size: 16px; margin-top: 10px;">
                该工具支持双语字幕生成和视频重组，让创作过程更加高效和富有创意。
            </p>
        </div>
        """)
        ...
        # 描述
        desc_label = QLabel("AI视频处理与字幕生成工具开发")
...
class TechDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        <div style="margin: 20px;">
            <h3>核心技术</h3>
            <p>VisionAI-ClipsMaster 使用了以下核心技术:</p>
            <ul>
                <li><b>大型语言模型</b>: 基于Mistral-7B和Qwen3系列的自然语言处理</li>
                <li><b>视频分析</b>: 基于深度学习的视频帧分析和情感识别</li>
                <li><b>音频处理</b>: 基于Librosa的音频分析和处理</li>
                <li><b>NLP处理</b>: 自然语言处理技术实现智能文本生成</li>
            </ul>

            <h3>系统架构</h3>
            <p>系统采用模块化设计，主要包含以下部分:</p>
            <ul>
                <li>视频处理核心引擎</li>
                <li>NLP语言处理模块</li>
                <li>模型训练和优化模块</li>
...
class HistoryDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        <div style="margin: 20px;">
            <h3>2025年3月 - 项目构想</h3>
            <p>VisionAI-ClipsMaster项目最初构想形成，开始研究AI视频处理技术。</p>

            <h3>2025年3月中旬 - 技术原型</h3>
            <p>完成核心算法研发，构建基础模型训练框架。</p>

            <h3>2025年4月初 - 功能开发</h3>
            <p>实现基本视频处理和字幕生成功能，完成界面设计。</p>

            <h3>2025年4月下旬 - 测试阶段</h3>
            <p>完成软件内部测试，修复已知问题，优化用户体验。</p>

            <h3>2025年5月 - 功能完善</h3>
            <p>增加爆款字幕生成和模型训练功能，实现双语支持。</p>
        </div>
        """)
        layout.addWidget(content)
