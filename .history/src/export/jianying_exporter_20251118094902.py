...
class JianyingExporter(BaseExporter):
    ...
    def _create_draft_file(self, version: Dict[str, Any], output_path: str) -> None:
        ...
        for i, scene in enumerate(scenes):
            ...
            #[object Object]修复：正确区分原视频时间码和新视频时间码
            # original_start_time/original_end_time 是原视频的绝对时间（用于素材定位）
            # start_time/end_time 是新视频的相对时间（用于时间轴排列）

            # 原视频的时间码（素材的起止点）
            if 'original_start_time' in scene and 'original_end_time' in scene:
                material_start = float(scene['original_start_time'])
                material_end = float(scene['original_end_time'])
                duration = material_end - material_start
            elif 'start_time' in scene and 'end_time' in scene:
                # 兼容旧格式
                material_start = float(scene.get('start_time', i * 5))
                material_end = float(scene.get('end_time', (i + 1) * 5))
                duration = material_end - material_start
            else:
                # 默认值
                material_start = float(scene.get('start_time', i * 5))
                duration = float(scene.get('duration', 5))
                material_end = material_start + duration

            # 创建视频片段
            video_segment = {
                "id": str(uuid.uuid4()),
                "material_id": draft["materials"]["videos"][0]["id"] if draft["materials"]["videos"] else "",
                "start_time": material_start,  # 原视频中的起始位置
                "duration": duration,          # 片段时长
                "target_timerange": {
                    "start": current_time,     # 新视频中的起始位置（连续的）
                    "duration": duration       # 片段时长
                }
            }
