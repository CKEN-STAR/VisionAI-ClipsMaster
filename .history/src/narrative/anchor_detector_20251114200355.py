...
def get_top_anchors(anchors: List[AnchorInfo], k: int = 5) -> List[AnchorInfo]:
    """
    按置信度选取前k个锚点（兼容API层调用）。
    """
    try:
        anchors_sorted = sorted(anchors, key=lambda a: getattr(a, "confidence", 0.0), reverse=True)
        return anchors_sorted[:k]
    except Exception:
        return anchors[:k]


def visualize_anchors(anchors: List[AnchorInfo], total_duration: float, output_path) -> Dict[str, Any]:
    """
    生成简单可视化数据并保存为JSON，避免引入重依赖。
    返回值作为API的data字段。
    """
    try:
        ...
        vis = {
            ...
            "anchors": [
                {
                    **{k: (v.value if hasattr(v, "value") else v) for k, v in asdict(a).items() if k != "anchor_type"},
                    "type": getattr(getattr(a, "anchor_type", None), "value", str(getattr(a, "anchor_type", None)))
                }
                for a in anchors
            ],
        }
        ...
        return {"total_duration": float(total_duration or 0.0), "anchors": []}
