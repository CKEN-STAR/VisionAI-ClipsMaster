...
class SimpleScreenplayApp(QMainWindow):
    ...
    def check_models(self):
        ...
        zh_model_paths = [
            ...
            project_root / "models" / "qwen" / "base" / "qwen2.5-7b.bin",
            project_root / "models" / "qwen" / "base" / "qwen2.5-7b",
            project_root / "models" / "qwen" / "finetuned",
            project_root / "models" / "qwen2.5-1.5b" / "fp16",  # 新增：检查新下载的模型
            project_root / "models" / "qwen2.5-1.5b" / "int4",
            project_root / "models" / "qwen2.5-1.5b" / "int8",
        ]

        # 检查models/qwen目录是否存在并有实际模型文件
        qwen_dir = project_root / "models" / "qwen"
        qwen2_5_dir = project_root / "models" / "qwen2.5-1.5b"  # 新增：检查新模型目录
        ...
        if not self.zh_model_exists:
            ...
            elif qwen2_5_dir.is_dir():
                self.zh_model_exists = self._has_large_files(str(qwen2_5_dir))
