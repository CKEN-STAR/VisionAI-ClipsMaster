...
# JSON 序列化容错（处理 numpy 标量/数组、集合、时间类型等）
from datetime import datetime as _dt_for_json
...
class QualityProbe:
    ...
    def analyze_narrative_coherence(self,
        ...
        # 分析相邻字幕在“输出时间轴”上的连续性（优先使用 output_*，否则回退到原始时间轴）
        time_gaps: List[float] = []
        use_output_axis = any(('output_start' in s and 'output_end' in s) for s in subtitle_segments)
        for i in range(1, len(subtitle_segments)):
            prev = subtitle_segments[i-1]
            curr = subtitle_segments[i]
            prev_end = (prev.get('output_end') if use_output_axis else prev.get('end_time', 0))
            curr_start = (curr.get('output_start') if use_output_axis else curr.get('start_time', 0))
            if prev_end is not None and curr_start is not None:
                try:
                    gap = float(curr_start) - float(prev_end)
                except Exception:
                    gap = 0.0
                time_gaps.append(max(0.0, gap))  # 只考虑正间隔

        # 计算平均间隔并归一化
        if time_gaps:
            avg_gap = sum(time_gaps) / len(time_gaps)
            # 理想间隔应小于0.5秒（输出轴上通常为0，表示无缝拼接）
            coherence_score = max(0.0, 1.0 - (avg_gap / 2.0))
        else:
            # 若无法计算间隔但存在多段，输出轴通常意味着无缝拼接，给出中高默认值
            coherence_score = 0.8 if use_output_axis and len(subtitle_segments) >= 2 else 0.5
    ...
    def evaluate_quality(self,
        ...
        # 计算平均分数（转为原生Python float，避免JSON序列化问题）
        overall_metrics['audio_quality'] = float(np.mean(overall_metrics['audio_quality']))
        overall_metrics['visual_quality'] = float(np.mean(overall_metrics['visual_quality']))
        ...
        self.probe_data['summary'] = {
            ...
            'pass': bool(float(overall_score) >= 0.7)  # 总体质量阈值
        }
    ...
    def _save_report(self) -> str:
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            json.dump(self.probe_data, f, ensure_ascii=False, indent=2, default=_json_default)
