...
def create_multi_tier_download_config() -> Dict:
    """
    创建多层级下载配置（支持LoRA/QLoRA微调）

    注意：
    - FP16: 原始HuggingFace模型，未量化，适合高配设备
    - INT8: HuggingFace格式的INT8量化模型（可能使用GPTQ或bitsandbytes量化）
    - INT4: HuggingFace格式的INT4量化模型（可能使用GPTQ或bitsandbytes量化）
    - 所有格式都支持LoRA/QLoRA微调
    """
    return {
        "qwen3-4b": {
