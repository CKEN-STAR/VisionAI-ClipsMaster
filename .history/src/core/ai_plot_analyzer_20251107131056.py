...
核心功能：
1. 基于双模型架构（Qwen3系列中文/Mistral-7B英文）实现完整叙事图谱构建
...
class AIPlotAnalyzer:
    ...
    def __init__(self):
        ...
        # LLM 调用预算与计数器（避免大规模真实素材时资源耗尽/卡死）
        self._llm_calls = 0
        self._emotion_calls = 0
        self._narrative_calls = 0
        self._character_calls = 0
        self.max_total_llm_calls = int(os.getenv("VACL_MAX_LLM_CALLS", "200"))
        self.max_emotion_llm_calls = int(os.getenv("VACL_MAX_EMOTION_CALLS", "120"))
        self.max_narrative_llm_calls = int(os.getenv("VACL_MAX_NARRATIVE_CALLS", "40"))
        self.max_character_llm_calls = int(os.getenv("VACL_MAX_CHARACTER_CALLS", "60"))
        cfg = os.getenv("VACL_MEMORY_LIMIT_MB")
        if cfg:
            self.memory_limit_mb = int(cfg)
        else:
            try:
                from src.config.env_adapter import get_optimal_memory_limit  # 延迟导入避免循环依赖
                self.memory_limit_mb = int(get_optimal_memory_limit())
            except Exception:
                if psutil is not None:
                    self.memory_limit_mb = int(psutil.virtual_memory().total / (1024 * 1024) * 0.70)
                else:
                    self.memory_limit_mb = 3500  # 保守回退

        logger.info(
            "AI剧情分析器初始化完成 (LLM预算: total=%d, emotion=%d, narrative=%d, character=%d, mem_limit=%dMB)",
            self.max_total_llm_calls, self.max_emotion_llm_calls, self.max_narrative_llm_calls,
            self.max_character_llm_calls, self.memory_limit_mb
        )

    def _should_use_llm(self) -> bool:
        """在内存压力或超出预算时关闭LLM调用，回退到规则方法"""
        # 预算检查
        if self._llm_calls >= self.max_total_llm_calls:
            logger.warning(
                "LLM总调用预算已用尽: %d/%d，回退到规则方法",
                self._llm_calls, self.max_total_llm_calls
            )
            return False
        # 内存压力检查
        try:
            if psutil is not None:
                ...
                if rss_mb >= self.memory_limit_mb * 0.95:
                    logger.warning(
                        "检测到内存压力 (%.1fMB/%dMB)，暂停LLM调用，使用规则方法",
    ...
    def _select_step(self, total: int, budget: int) -> int:
        """根据预算选择步长，确保调用次数有上限"""
        try:
            return max(1, math.ceil(total / max(budget, 1)))
        except Exception:
            return max(1, total or 1)

    def _count_llm(self, scope: str = "total") -> None:
        """统计一次LLM调用"""
        try:
            self._llm_calls += 1
            if scope == "emotion":
                self._emotion_calls += 1
            elif scope == "narrative":
                self._narrative_calls += 1
            elif scope == "character":
                self._character_calls += 1
        except Exception:
            pass
    ...
    def _analyze_emotion_curve(self, subtitles: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            step = self._select_step(len(subtitles), self.max_emotion_llm_calls)
            ...
            for i, subtitle in enumerate(subtitles):
                try:
                    ...
                    use_ai = bool(llm) and (i % step == 0) and self._should_use_llm() and (self._emotion_calls < self.max_emotion_llm_calls)
                    if use_ai:
                        emotion_analysis = self._ai_analyze_emotion(llm, text, language)
                        self._count_llm("emotion")
                    else:
                        # 回退到规则方法
            ...
            logger.info(
                f"情感曲线分析完成(步长={step}, LLM调用={self._emotion_calls}/{self.max_emotion_llm_calls})，共{len(emotion_curve)}个情感点"
            )
    ...
    def _identify_narrative_structure(self, subtitles: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            step = self._select_step(len(segments), self.max_narrative_llm_calls)
            ...
            for i, segment in enumerate(segments):
                try:
                    ...
                    # 使用AI分析叙事阶段（采样 + 内存/预算门控）
                    use_ai = bool(llm) and (i % step == 0) and self._should_use_llm() and (self._narrative_calls < self.max_narrative_llm_calls)
                    if use_ai:
                        narrative_analysis = self._ai_analyze_narrative(llm, segment_text, language)
                        self._count_llm("narrative")
                    else:
                        # 回退到规则方法
            ...
            logger.info(
                f"叙事结构识别完成(步长={step}, LLM调用={self._narrative_calls}/{self.max_narrative_llm_calls})，共{len(plot_points)}个情节点"
            )
            return plot_points
    ...
    def _analyze_characters(self, subtitles: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            characters = []
            step_char = self._select_step(len(subtitles), self.max_character_llm_calls)
            ...
            for i, subtitle in enumerate(subtitles):
                try:
                    ...
                    # 使用AI识别角色（采样 + 预算/内存门控）
                    use_ai = bool(llm) and (i % step_char == 0) and self._should_use_llm() and (self._character_calls < self.max_character_llm_calls)
                    if use_ai:
                        character_analysis = self._ai_analyze_characters(llm, text, language)
                        self._count_llm("character")
                    else:
                        # 回退到规则方法
            ...
            logger.info(f"角色分析完成(LLM调用={self._character_calls}/{self.max_character_llm_calls})，识别出{len(characters)}个角色")
            return characters
    ...
    def _is_low_quality_text(self, text: str) -> bool:
        ...
        try:
            ...
            total = len(tokens)
            if total >= 50:
                uniq_ratio = len(set(tokens)) / float(total)
                if uniq_ratio < 0.2 and len(text) > 300:
                    return True
    ...
    def _validate_emotion_result(self, d: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """校验/修正情感分析结果"""
        try:
            if not isinstance(d, dict):
                return None
            etype = str(d.get("type", "neutral")).lower()
            if etype not in {"positive", "negative", "neutral", "intense", "suspense"}:
                etype = "neutral"
            ...
            return {"intensity": intensity, "type": etype, "confidence": confidence}
        except Exception:
            return None

    def _validate_narrative_result(self, d: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """校验/修正叙事分析结果"""
        try:
            if not isinstance(d, dict):
                return None
            stage = str(d.get("stage", "DEVELOPMENT")).upper()
            allowed = {"SETUP", "DEVELOPMENT", "TWIST", "CONCLUSION"}
            if stage not in allowed:
                stage = "DEVELOPMENT"
            importance = float(d.get("importance", 0.5))
            if not (0.0 <= importance <= 1.0):
                importance = max(0.0, min(1.0, importance))
            description = str(d.get("description", ""))[:40]
            keywords = d.get("keywords", [])
            if not isinstance(keywords, list):
                keywords = []
            keywords = [str(k)[:10] for k in keywords[:5]]
            return {"stage": stage, "importance": importance, "description": description, "keywords": keywords}
        except Exception:
            return None

    def _validate_character_result(self, d: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """校验/修正角色分析结果：保证返回 {characters: List[str], actions: List[str]}"""
        try:
            if not isinstance(d, dict):
                return None
            ...
            if not isinstance(chars, list):
                chars = []
            chars = [str(c)[:20] for c in chars[:10]]

            acts = d.get("actions", [])
            if not isinstance(acts, list):
                acts = []
            acts = [str(a)[:30] for a in acts[:10]]

            return {"characters": chars, "actions": acts}
        except Exception:
            return None
    # --- 分段提示词/结果合并工具 ---
    def _split_text_head_tail(self, text: str, max_window_chars: int = 800) -> List[str]:
        """对过长文本做头尾窗口切分，避免超长上下文。
        返回1-2个窗口；尽量控制额外LLM调用次数。
        """
        try:
            if not text:
                return [""]
            ...
            return [text[:max_window_chars]]

    def _merge_emotion_results(self, parts: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """合并多窗口的情感结果：强度取平均，类型取众数，置信度取平均。"""
        try:
            parts = [p for p in parts if p]
            if not parts:
                return None
            intensity = sum(p.get("intensity", 0.5) for p in parts) / len(parts)
            confidence = sum(p.get("confidence", 0.5) for p in parts) / len(parts)
            # 众数类型
            types = {}
            for p in parts:
                t = p.get("type", "neutral")
                types[t] = types.get(t, 0) + 1
            type_final = max(types.items(), key=lambda x: x[1])[0]
            return {"intensity": float(max(0.0, min(1.0, intensity))), "type": type_final, "confidence": float(max(0.0, min(1.0, confidence)))}
        except Exception:
            return None

    def _merge_narrative_results(self, parts: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """合并叙事结果：阶段取众数或重要性最高；重要性取平均；关键词去重取前5。"""
        try:
            parts = [p for p in parts if p]
            if not parts:
                return None
            # 阶段统计与重要性
            stage_counts = {}
            best_by_importance = (None, -1.0, {})
            total_importance = 0.0
            all_keywords = []
            descriptions = {}
            for p in parts:
                st = p.get("stage", "DEVELOPMENT")
                stage_counts[st] = stage_counts.get(st, 0) + 1
                imp = float(p.get("importance", 0.5))
                total_importance += imp
                if imp > best_by_importance[1]:
                    best_by_importance = (st, imp, p)
            ...
            # 阶段优先众数，其次重要性最高
            stage_final = max(stage_counts.items(), key=lambda x: x[1])[0] if stage_counts else best_by_importance[0]
            importance_final = max(0.0, min(1.0, total_importance / len(parts)))
            # 关键词去重保序
            seen = set(); merged_kws = []
            ...
            if descriptions:
                description_final = max(descriptions.items(), key=lambda x: x[1])[0]
            else:
                description_final = str(best_by_importance[2].get("description", "")) if best_by_importance[2] else ""
            return {"stage": stage_final, "importance": importance_final, "description": description_final[:40], "keywords": merged_kws}
        except Exception:
            return None

    def _merge_character_results(self, parts: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """合并角色结果：名称与动作取并集，限制数量。"""
        try:
            parts = [p for p in parts if p]
            if not parts:
                return None
            names = []
            actions = []
            seen_n = set(); seen_a = set()
            for p in parts:
                for n in p.get("characters", []) or []:
                    n = str(n).strip()
                ...
                for a in p.get("actions", []) or []:
                    ...
                    if a and a not in seen_a:
                        seen_a.add(a); actions.append(a)
                        if len(actions) >= 10:
                            break
            return {"characters": names[:10], "actions": actions[:10]}
        except Exception:
            return None
            ...
            if not isinstance(acts, list):
                acts = []
            chars = [str(c).strip()[:20] for c in chars[:10] if str(c).strip()]
            acts = [str(a).strip()[:40] for a in acts[:10] if str(a).strip()]
            return {"characters": chars, "actions": acts}
        except Exception:
            return None
...
Output a strict single JSON only:
{"intensity": 0.0, "type": "neutral", "confidence": 0.0}
"""

            # 严格JSON约束：禁止Markdown围栏/解释文字
            if language == "zh":
                prompt += "\n不要使用Markdown代码块或```json```围栏。输出必须可被json.loads解析。\n"
            else:
                prompt += "\nDo not use Markdown code fences or ```json``` fenced blocks. Output must be a valid JSON object parseable by json.loads.\n"

            # 分段提示词：对超长文本使用头尾窗口并合并结果
            windows = self._split_text_head_tail(text, max_window_chars=1000)
            if len(windows) > 1:
                part_results = []
                for w in windows:
                    if language == "zh":
...
"""
                    ...
                    if language == "zh":
                        ...
                        p += "\nDo not use Markdown code fences or ```json``` fenced blocks. Output must be parseable by json.loads.\n"

                    max_attempts = 2
                    for attempt in range(1, max_attempts + 1):
                        response = llm.generate(p, temperature=0.25, max_length=200, json_only=True)
                        if self._is_low_quality_text(response):
                            logger.warning("AI情感分析(窗口)输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                            if attempt >= max_attempts:
                                break
                            continue
                        data = self._extract_json_dict(response)
                        result = self._validate_emotion_result(data or {}) if data else None
                        if result is not None:
                            part_results.append(result)
                            break
                merged = self._merge_emotion_results(part_results)
            ...
            max_attempts = 2
            for attempt in range(1, max_attempts + 1):
                response = llm.generate(prompt, temperature=0.25, max_length=200, json_only=True)
                if self._is_low_quality_text(response):
                    logger.warning("AI情感分析输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                    if attempt >= max_attempts:
                        break
                ...
                logger.warning("AI情感分析JSON解析失败，尝试重试 %d/%d", attempt, max_attempts)
                if attempt >= max_attempts:
                    break
...
"""
            ...
            # 分段提示词：对超长文本使用头尾窗口并合并结果
            windows = self._split_text_head_tail(text, max_window_chars=1200)
            if len(windows) > 1:
                part_results = []
                for w in windows:
                    if language == "zh":
...
"""
                    ...
                    max_attempts = 2
                    for attempt in range(1, max_attempts + 1):
                        response = llm.generate(p, temperature=0.3, max_length=300, json_only=True)
                        if self._is_low_quality_text(response):
                            logger.warning("AI叙事分析(窗口)输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                            if attempt >= max_attempts:
                                break
                            continue
                        data = self._extract_json_dict(response)
                        result = self._validate_narrative_result(data or {}) if data else None
                        if result is not None:
                            part_results.append(result)
                            break
                merged = self._merge_narrative_results(part_results)
                if merged is not None:
                    return merged
            ...
            max_attempts = 2
            for attempt in range(1, max_attempts + 1):
                response = llm.generate(prompt, temperature=0.3, max_length=300, json_only=True)
                if self._is_low_quality_text(response):
                    logger.warning("AI叙事分析输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                    if attempt >= max_attempts:
                        break
                    continue
                ...
                logger.warning("AI叙事分析JSON解析失败，尝试重试 %d/%d", attempt, max_attempts)
                if attempt >= max_attempts:
                    break
...
"""
            ...
            # 分段提示词：对超长文本使用头尾窗口并合并结果
            windows = self._split_text_head_tail(text, max_window_chars=900)
            if len(windows) > 1:
                part_results = []
                for w in windows:
                    if language == "zh":
...
"""
                    ...
                    max_attempts = 2
                    for attempt in range(1, max_attempts + 1):
                        response = llm.generate(p, temperature=0.3, max_length=200, json_only=True)
                        if self._is_low_quality_text(response):
                            logger.warning("AI角色分析(窗口)输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                            if attempt >= max_attempts:
                                break
                            continue
                        data = self._extract_json_dict(response)
                        result = self._validate_character_result(data or {}) if data else None
                        if result is not None:
                            part_results.append(result)
                            break
                merged = self._merge_character_results(part_results)
                if merged is not None:
                    return merged
            ...
            max_attempts = 2
            for attempt in range(1, max_attempts + 1):
                response = llm.generate(prompt, temperature=0.3, max_length=200, json_only=True)
                if self._is_low_quality_text(response):
                    logger.warning("AI角色分析输出疑似低质量/重复，尝试重试 %d/%d", attempt, max_attempts)
                    if attempt >= max_attempts:
                        break
                    continue
                ...
                logger.warning("AI角色分析JSON解析失败，尝试重试 %d/%d", attempt, max_attempts)
                if attempt >= max_attempts:
                    break
