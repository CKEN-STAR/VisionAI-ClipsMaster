...
# GPU相关导入：改为懒加载，避免在导入期触发底层 DLL 加载问题
TORCH_AVAILABLE = False

def _load_torch():
    """在首次使用时尝试加载 torch/torchvision；失败则提供占位实现"""
    global TORCH_AVAILABLE, torch, transforms
    if TORCH_AVAILABLE and 'torch' in globals():
