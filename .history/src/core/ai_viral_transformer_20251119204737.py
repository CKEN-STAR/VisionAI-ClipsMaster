...
"""
    ...
    def _optimize_viral_potential(self, segments: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # 关键修复：重建时间轴以确保连续性
            final_segments = self._rebuild_timeline(optimized_segments)
    ...
    def _rebuild_timeline(self, segments: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        重建时间轴，确保片段连续

        Args:
            segments: 按原始时间排序的片段列表
        ...
        try:
            ...
            for i, segment in enumerate(segments):
                ...
                if original_duration <= 0:
                    logger.warning(f"发现无效片段，时长为0，已跳过: {segment['text']}")
                    continue

                new_segment = segment.copy()
                new_segment["start_time"] = current_time
                new_segment["end_time"] = current_time + original_duration

                rebuilt_segments.append(new_segment)
                current_time += original_duration

            logger.info(f"时间轴重构完成，共 {len(rebuilt_segments)} 个片段，总时长: {current_time:.2f}秒")
            return rebuilt_segments

        except Exception as e:
            logger.error(f"时间轴重构失败: {str(e)}")
            return segments # 出错时返回原始片段
