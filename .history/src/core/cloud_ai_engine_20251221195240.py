...
class CloudAIEngine:
    ...
    def _ensure_narrative_continuity(self, selected_indices: List[int], episode_subs: List[Dict]) -> List[int]:
        ...
        策略：
        1. 如果选中的对话之间有小间隙（1-2条），自动补充
        2. 检测"因果关系"词汇，确保起因和结果都被选中
        3. 检测"转折/递进"词汇，确保前后文完整
        ...
        # 因果关系词汇（如果选中了包含这些词的对话，需要检查上下文）
        cause_words = ['因为', '由于', '所以', '因此', '于是', '结果', '导致', '原来', '怪不得']
        continuation_words = ['但是', '可是', '然而', '不过', '而且', '并且', '同时', '另外', '还有']
        question_words = ['为什么', '怎么', '什么', '哪里', '谁', '吗', '呢', '？']
        
        enhanced_indices = set(sorted_indices)
        
        # 策略1：填补小间隙（如果两个选中的对话之间只隔1-2条，自动补充）
            ...
            if 2 <= gap <= 3:
                ...
                for fill_idx in range(current_idx + 1, next_idx):
                    if fill_idx <= len(episode_subs):
                        ...
                        # 如果包含连接词或因果词，补充进来
                        if any(word in fill_text for word in cause_words + continuation_words):
                            enhanced_indices.add(fill_idx)
                        # 如果间隙只有1条，直接补充（保持连贯）
                        elif gap == 2:
                            enhanced_indices.add(fill_idx)
        
        # 策略2：检查因果关系，确保起因和结果都被选中
        for idx in sorted_indices:
            if idx <= len(episode_subs):
                text = episode_subs[idx - 1].get("text", "")
                
                # 如果当前对话包含"结果"类词汇，检查前面是否有"起因"
                if any(word in text for word in ['所以', '因此', '于是', '结果', '怪不得', '原来']):
                    # 向前查找2-3条，看是否有起因
                    for prev_idx in range(max(1, idx - 3), idx):
                        if prev_idx not in enhanced_indices:
                            prev_text = episode_subs[prev_idx - 1].get("text", "")
                            if any(word in prev_text for word in ['因为', '由于']) or len(prev_text) > 5:
                ...
                if any(word in text for word in question_words):
                    ...
                    for next_idx in range(idx + 1, min(len(episode_subs) + 1, idx + 3)):
                        if next_idx not in enhanced_indices and next_idx <= len(episode_subs):
                            enhanced_indices.add(next_idx)
                            break  # 只补充一条回答
                
                # 如果当前对话包含转折词，确保前文被选中
                if any(word in text for word in ['但是', '可是', '然而', '不过']):
                    prev_idx = idx - 1
                    if prev_idx >= 1 and prev_idx not in enhanced_indices:
                        enhanced_indices.add(prev_idx)
        
        # 策略3：确保每个选中片段的开头不是"半句话"
        final_indices = sorted(enhanced_indices)
        for idx in final_indices:
            if idx <= len(episode_subs):
                text = episode_subs[idx - 1].get("text", "")
                # 如果以连接词开头，说明需要前文
                if text and any(text.startswith(word) for word in ['但', '可', '然', '不过', '而', '所以', '因此']):
                    prev_idx = idx - 1
                    if prev_idx >= 1 and prev_idx not in enhanced_indices:
                        enhanced_indices.add(prev_idx)
        ...
        if added_count > 0:
            logger.info(f"叙事连贯性优化：补充了 {added_count} 条上下文对话")
...
请用300-500字概括整个故事。"""
        ...
        for ep_idx, episode_subs in enumerate(all_subtitles):
            ...
            # 🔧 优化：构建更注重叙事连贯性的提示词
            extract_prompt = f"""基于以下故事摘要，从第{ep_idx + 1}集的字幕中提取关键对话片段。
...
【重要】请选择这一集中最重要的对话片段（约占总数的30%-50%），选择时必须遵循以下原则：

1. **叙事连贯性优先**：
   - 如果选择了某个事件的"结果"，必须同时选择该事件的"起因"
   - 如果选择了某个冲突的"高潮"，必须同时选择"铺垫"部分
   - 连续的对话片段应该保持完整，不要只选中间部分

2. **完整的情节单元**：
   - 每个选中的片段应该是一个完整的情节单元
   - 避免选择孤立的、缺乏上下文的台词
   - 如果某句话需要前面的对话才能理解，请一起选择
...
请以JSON数组格式输出你选择的对话序号，例如：[1, 2, 3, 8, 9, 10, 15, 16]
注意：连续的序号表示这些对话是一个完整的情节单元，应该一起保留。
只输出JSON数组，不要其他内容。"""
