...
class WorkflowManager:
    ...
    def execute_full_workflow(self, video_path: str, subtitle_path: str,
        ...
        try:
            ...
            # 默认长度控制：若未指定，从配置载入（narrative_config.yaml / param_matrix.yaml）
            if target_length is None:
                try:
                    from src.utils.file_utils import safe_read_yaml
                    ncfg = safe_read_yaml("configs/narrative_config.yaml", {}) or {}
                    tcfg = ((ncfg.get("segment_selection") or {}).get("total_duration") or {})
                    tmin = int(tcfg.get("min", 30))
                    tmax = int(tcfg.get("max", 120))
                    pm = safe_read_yaml("configs/param_matrix.yaml", {}) or {}
                    g = pm.get("global") or {}
                    tmin = int(g.get("min_total_duration", tmin))
                    tmax = int(g.get("max_total_duration", tmax))
                    target_length = (tmin, tmax)
                    logger.info(f"📏 未指定目标长度，使用配置默认: {target_length[0]}-{target_length[1]}秒")
                except Exception as e:
                    logger.warning(f"长度配置加载失败，跳过长度控制: {e}")
            ...
            # 智能自适应目标时长（Auto）：基于对白密度/情感/原片时长
            try:
                auto_flag = str(os.getenv("VACL_AUTO_LENGTH", "1")).lower() in ("1", "true", "yes")
                if auto_flag:
                    subs = parsing_result.get("subtitles", [])
                    total_dur = float(parsing_result.get("total_duration", 0.0) or 0.0)
                    n = len(subs)
                    dpm = (n / (total_dur / 60.0)) if total_dur > 0 else 0.0  # 每分钟条数
                    emo = float(analysis_result.get("emotional_score", 0.7) or 0.7)
                    scene_cnt = int(analysis_result.get("scene_count", 0) or 0)

                    # 简洁鲁棒的分档：S长版 / M中版 / LITE短版
                    grade = "LITE"
                    if total_dur >= 480 and dpm >= 28 and emo >= 0.75:
                        grade = "S"
                    elif total_dur >= 300 and (dpm >= 20 or emo >= 0.65):
                        grade = "M"
                    ...
                    # 不超过原片时长的95%-98%
                    tmax = min(tmax, int(max(60, total_dur * 0.98)))
                    tmin = min(tmin, tmax - 10) if tmax > 70 else tmax - 10

                    target_length = (int(tmin), int(tmax))
                    os.environ["VACL_DIALOGUE_RETAINED_PCT"] = str(rr)
                    os.environ["VACL_MAX_SEGMENTS"] = str(ms)
                    logger.info(f"🧠 自动目标长度: 档位={grade}｜dpm={dpm:.1f}｜emo={emo:.2f}｜scenes={scene_cnt}｜range={target_length[0]}-{target_length[1]}s｜retain={rr}｜maxSeg={ms}")
            except Exception as e:
                logger.warning(f"自动长度计算失败，使用原配置: {e}")
    ...
    def _generate_clips(self, video_path: str, subtitles: List[Dict],
        ...
        final_duration = self.clip_generator._get_video_duration(output_video)
    ...
    def auto_calibrate_metrics(self, total_duration: float, subtitle_count: int, emotional_score: float) -> Dict[str, Any]:
        """基于聚合指标进行自动档位与参数推断（UI批量SRT场景复用）

        说明：
        - 不依赖视频文件，仅使用总时长/字幕条数/情感得分
        - 内部沿用 execute_full_workflow 中的分档与ENV下发逻辑
        - 不做任何“上限裁剪”，只设置建议区间与保留比例

        Args:
            total_duration: 所有原字幕累计时长（秒）（多集取"求和"）
            subtitle_count: 总字幕条数（多集取"求和"）
            emotional_score: 聚合情感强度（建议按时长加权平均）
        Returns:
            dict: {grade, target_min, target_max, retain_ratio, max_segments, dpm}
        """
        import os
        try:
            # 安全边界
            ...
            emotional_score = float(emotional_score or 0.7)

            # 每分钟字幕条数（对白密度）
            dpm = (subtitle_count / (total_duration / 60.0)) if total_duration > 0 else 0.0

            # 自动开关（默认启用）
            auto_flag = str(os.getenv("VACL_AUTO_LENGTH", "1")).lower() in ("1", "true", "yes")
            grade = "LITE"
            tmin, tmax, rr, ms = 150, 240, 0.60, 120

            if auto_flag:
                if total_duration >= 480 and dpm >= 28 and emotional_score >= 0.75:
                    grade = "S"; tmin, tmax, rr, ms = 480, 600, 0.75, 320
                elif total_duration >= 300 and (dpm >= 20 or emotional_score >= 0.65):
                    grade = "M"; tmin, tmax, rr, ms = 300, 420, 0.70, 200

                # 不超过素材总时长的98%，下限贴近上限防止负区间
                tmax = min(tmax, int(max(60, total_duration * 0.98)))
                tmin = min(tmin, tmax - 10) if tmax > 70 else max(0, tmax - 10)

                # 下发建议参数到ENV，供RealAIEngine/SegmentAdvisor等读取
                os.environ["VACL_DIALOGUE_RETAINED_PCT"] = str(rr)
                os.environ["VACL_MAX_SEGMENTS"] = str(ms)

                logger.info(
                    f"🧠 自动目标长度(UI批量): 档位={grade}｜dpm={dpm:.1f}｜emo={emotional_score:.2f}｜"
                    f"range={tmin}-{tmax}s｜retain={rr}｜maxSeg={ms}"
                )
            ...
            logger.warning(f"自动长度计算失败(UI批量)，使用默认: {e}")
            return {
                "grade": "LITE",
                "target_min": 150,
                "target_max": 240,
