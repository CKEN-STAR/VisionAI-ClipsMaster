...
class ClipGenerator:
    ...
    def _process_segments(self, video_path: str, segments: List[Dict[str, Any]],
        ...
        try:
            ...
            if merge_short and len(valid_segments) > 1:
                ...
                try:
                    logger.info(f"[Merge] before={before_cnt}, after={len(valid_segments)}, gap={gap_thres}, max={max_merged}, min={min_dur}, tail_pad={tail_pad}, punct_factor={punct_merge_factor}")
                except Exception:
                    pass
            ...
            for i, item in enumerate(valid_segments):
                ...
                # 末尾安全缓冲：若文本末尾无句末标点，尝试向后微扩展，避免“话未完即切”
                try:
                    if tail_pad > 0:
    ...
    def _merge_and_filter_segments(self,
                                   ...
                                   min_non_punct_gap_merge: float = 0.0) -> List[Dict[str, Any]]:
        """
            ...
            if not can_merge:
                try:
                    ...
                    if not ends_with_punct and ep_ok:
                        ...
                        if (gap <= allow_gap) and (would_merged_len <= max_merged):
                            can_merge = True
                except Exception:
                    pass
