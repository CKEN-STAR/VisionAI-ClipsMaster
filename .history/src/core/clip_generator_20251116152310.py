...
class ClipGenerator:
    ...
    def _process_segments(self, video_path: str, segments: List[Dict[str, Any]],
        ...
        try:
            ...
            try:
                ...
                logger.info(f"[ProcCfg] min_dur={min_dur}, gap_merge={gap_thres}, max_merge={max_merged}, tail_pad={tail_pad}, no_punct_extra={proc.get('no_punct_tail_extra', 0.10)}, head_min_gap={head_min_gap}")
            except Exception:
                pass
            ...
            for i, item in enumerate(valid_segments):
                ...
                try:
                    ...
                    # 预留极小安全间隙，避免与下一段重叠
                    safe_next = max(nxt_start - 0.02, end_time)
                    ...
                    # 不与下一段发生重叠
                    end_time = min(target_end, safe_next)

                    # 记录剪辑计划日志，便于定位边界
                    try:
                        final_duration = max(0.0, end_time - start_time)
                        extra_tag = f"+{extra_no_punct:.2f}" if not ends_with_punct else ""
                        logger.info(f"[CutPlan] seg#{i:03d} {start_time:.3f}->{orig_end:.3f} -> {end_time:.3f} dur={final_duration:.3f} pad={univ_pad:.2f}{extra_tag} next={nxt_start:.3f} safe={safe_next:.3f} punct={ends_with_punct}")
                    except Exception:
                        pass
                except Exception:
                    pass
    ...
    def _cut_segment(self, input_path: str, output_path: str,
        ...
        try:
            ...
            proc = self.config.get('processing', {}) or {}
            precise_cut = bool(proc.get('precise_cut', False))
            end_ts = start_time + duration
