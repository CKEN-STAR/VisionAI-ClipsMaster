    ...
    def __init__(self, use_gpu: bool = True, progress_callback=None):
        ...
        try:
            ...
            if FFMPEG_PATH:
                ...
                logger.info(f"FFmpeg路径已从环境配置加载: {self.ffmpeg_cmd}")
                ...
                if os.path.exists(candidate):
                    self.ffprobe_cmd = candidate
                else:
                    base = os.path.basename(self.ffmpeg_cmd)
                    if 'ffmpeg' in base:
                        self.ffprobe_cmd = self.ffmpeg_cmd.replace('ffmpeg.exe', 'ffprobe.exe').replace('ffmpeg', 'ffprobe')
