...
class IntelligentModelSelector:
    ...
    def recommend_model_version(
        ...
        valid_models = [
            "qwen3-0.6b", "qwen3-1.7b", "qwen3-8b", "qwen3-32b",  # Qwen3系列
        ...
        if recommendation and recommendation.variant is not None:
        ...
        if recommendation and recommendation.variant:
            logger.info(f"✅ 推荐完成: {recommendation.variant.name}")
        else:
            logger.info(f"⚠️  推荐完成: 设备硬件不足，未推荐任何变体")
    ...
    def _select_qwen_variant(self, hardware: HardwareProfile) -> str:
        ...
        模型实际大小（FP16）：
        - Qwen3-0.6B: ~1.2GB (min 2GB RAM/VRAM)
        - Qwen3-1.7B: ~3.4GB (min 6GB RAM/VRAM)
        - Qwen3-8B: ~16GB (min 16GB VRAM)
        - Qwen3-32B: ~64GB (min 48GB VRAM)
        """
        score = self.calculate_hardware_score(hardware)

        # 获取可用内存（GPU优先，否则使用系统内存）
        available_memory = hardware.gpu_memory_gb if hardware.has_gpu else hardware.system_ram_gb

        # 基于实际内存需求的智能匹配
        # 优先考虑内存容量，其次考虑硬件评分
        if available_memory >= 48 and score >= 75:
            return "qwen3-32b"      # 48GB+ VRAM, 75-100分：旗舰级
        elif available_memory >= 16 and score >= 50:
            return "qwen3-8b"       # 16GB+ VRAM, 50-75分：专业级
        elif available_memory >= 6 and score >= 30:
            return "qwen3-1.7b"     # 6GB+ RAM/VRAM, 30-50分：进阶级
        else:
            return "qwen3-0.6b"     # 2GB+ RAM/VRAM, 0-30分：入门级
...
def create_multi_tier_download_config() -> Dict:
    ...
    重要说明：
    - 只下载原始FP16模型，不下载预量化模型（INT8/INT4）
    - 原因：预量化模型无法正确转换为GGUF格式
    - 正确流程：FP16模型 + LoRA训练 → 保存HF格式 → 转换为GGUF（此时进行量化）
    - 量化在GGUF转换阶段进行，而不是在下载阶段

    更新（2025-01-31）：
    - 全面升级到Qwen3系列（0.6B, 1.7B, 8B, 32B）
    - 移除旧版本模型和Qwen3-4B（不在推荐列表中）
    """
    return {
        "qwen3-0.6b": {
            "fp16": {
                ...
                "size_gb": 1.2,
                "urls": [
                    "https://modelscope.cn/models/qwen/Qwen3-0.6B-Instruct",
                    "https://hf-mirror.com/Qwen/Qwen3-0.6B-Instruct",
        ...
        "qwen3-1.7b": {
            "fp16": {
                ...
                "size_gb": 3.4,
                "urls": [
                    "https://modelscope.cn/models/qwen/Qwen3-1.7B-Instruct",
                    "https://hf-mirror.com/Qwen/Qwen3-1.7B-Instruct",
        ...
        "qwen3-8b": {
            "fp16": {
                ...
                "size_gb": 16.0,
                "urls": [
                    "https://modelscope.cn/models/qwen/Qwen3-8B-Instruct",
                    "https://hf-mirror.com/Qwen/Qwen3-8B-Instruct",
        ...
        "qwen3-32b": {
            "fp16": {
                ...
                "urls": [
                    ...
                    "https://hf-mirror.com/Qwen/Qwen3-32B-Instruct",
                    "https://huggingface.co/Qwen/Qwen3-32B-Instruct"
                ],
        ...
        "mistral-7b": {
            "fp16": {
                ...
                "size_gb": 13.5,
                "urls": [
                    "https://modelscope.cn/models/mistralai/Mistral-7B-Instruct-v0.3",
                    "https://hf-mirror.com/mistralai/Mistral-7B-Instruct-v0.3",
