...
class EnhancedModelSwitcher:
    ...
    def __init__(self):
        ...
        # 模拟模型实例
        self.chinese_model = MockModel("Qwen3-1.7B-zh", "zh")
...
def switch_to_language(language: str) -> bool:
    """向后兼容的语言切换函数"""
    return _get_model_switcher().switch_to_language(language)

def generate_viral_content(text: str, **kwargs) -> Dict[str, Any]:
    """向后兼容的内容生成函数"""
    return _get_model_switcher().generate_viral_content(text, **kwargs)

def get_model_status() -> Dict[str, Any]:
    """获取模型状态"""
    return _get_model_switcher().get_model_status()
