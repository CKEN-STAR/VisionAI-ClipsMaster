...
# 惰性导入 spaCy，避免在导入期触发底层 DLL 加载
SPACY_AVAILABLE = False

def _maybe_import_spacy() -> bool:
    ...
    try:
        ...
        SPACY_AVAILABLE = False
        logger.warning("spaCy不可用（已降级）：将使用内置启发式语言检测")
        return False
...
class LanguageDetector:
    ...
    def detect_subtitle_language(self, subtitle_file: str) -> str:
        """向后兼容：从字幕文件路径检测语言
        等价于 detect_from_file("subtitle.srt")
        """
        return self.detect_from_file(subtitle_file)
    ...
    def get_model_for_language(self, language: str) -> str:
        """
        根据语言返回推荐的模型名称（用于日志/展示）
        优先读取 configs/model_config.yaml 的 default_models -> models[name]
        失败时回退至内置默认
        """
        try:
            # 
            ...
            default_models = (config or {}).get("default_models", {})
            models = (config or {}).get("models", {})
            model_id = default_models.get(language) or default_models.get("zh") or "qwen3-1.7b-zh"
            info = models.get(model_id, {})
            # 
            return info.get("name") or model_id
        except Exception:
            # 
            return "Qwen3-1.7B-Instruct" if (language or "zh").startswith("zh") else "Mistral-7B-Instruct-v0.2"
