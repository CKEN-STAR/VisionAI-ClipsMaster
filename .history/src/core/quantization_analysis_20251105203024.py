...
class QuantizationType(Enum):
    """量化类型枚举（支持LoRA/QLoRA微调）"""
    FP16 = "fp16"           # 16位浮点 (半精度)
    INT8 = "int8"           # 8位量化，group_size=128
    INT4 = "int4"           # 4位量化，group_size=128
    INT8_PERCHANNEL = "int8_perchannel"  # 8位per-channel量化
    INT4_PERCHANNEL = "int4_perchannel"  # 4位per-channel量化
...
class QuantizationAnalyzer:
    ...
    def _initialize_model_variants(self) -> Dict[str, List[ModelVariant]]:
        ...
        重要说明：
        - 只使用FP16原始模型，不使用预量化模型（INT8/INT4）
        - 原因：训练器使用AutoModelForCausalLM.from_pretrained，不支持预量化模型加载
        - 正确流程：FP16模型 + LoRA训练 → 保存HF格式 → 转换为GGUF（此时量化）
        - 量化在GGUF转换阶段进行（Q4_K_M, Q5_K, Q8_0等），而不是在下载阶段

        模型选择基于硬件评分：
        - 0-35分（入门级）：Qwen3-0.6B / Mistral-7B
        - 35-55分（进阶级）：Qwen3-1.7B / Mistral-7B
        - 55-75分（中高级）：Qwen3-8B / Mistral-7B
        - 75-100分（旗舰级）：Qwen3-32B / Mistral-7B
        """
        return {
            # Qwen3-0.6B 中文模型（入门级）- 仅FP16
            "qwen3-0.6b": [
                ModelVariant(
                    name="Qwen3-0.6B-Instruct-FP16",
                    quantization=QuantizationType.FP16,
                    size_gb=1.2,  # FP16原始模型大小
...
class HardwareDetector:
    ...
    def detect_hardware() -> HardwareProfile:
        ...
        # 检测GPU（增强版：优先GPUtil，其次torch.cuda）
        gpu_memory_gb = 0.0
        has_gpu = False
        gpu_compute_capability = None

        # 优先尝试 GPUtil（能直接拿到显存总量）
        try:
            ...
            if gpus:
                has_gpu = True
                try:
                    gpu_memory_gb = sum(getattr(g, "memoryTotal", 0.0) for g in gpus) / 1024.0
                except Exception:
                    gpu_memory_gb = max(getattr(gpus[0], "memoryTotal", 0.0), 0.0) / 1024.0
        except Exception as e:
            logger.debug(f"GPUtil检测失败: {e}")

        # 回退到 torch.cuda
        if not has_gpu or gpu_memory_gb <= 0:
            try:
                import torch
                if hasattr(torch, 'cuda') and hasattr(torch.cuda, 'is_available'):
                    if callable(torch.cuda.is_available) and torch.cuda.is_available():
                        try:
                            ...
                            logger.warning(f"CUDA可用但设备不可访问: {e}")
                            has_gpu = False
            except ImportError:
                pass
