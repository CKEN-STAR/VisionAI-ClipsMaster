...
# 🆕 导入组件卸载管理器
try:
    from src.core.unload_manager import UnloadManager
    UNLOAD_MANAGER_AVAILABLE = True
except ImportError:
    UNLOAD_MANAGER_AVAILABLE = False
...
class ClipGenerator:
    ...
    def __init__(self, use_gpu: bool = True):
        ...
        if CACHE_MANAGER_AVAILABLE:
            try:
                self.cache_manager = CacheManager(
                    max_memory_size=100 * 1024 * 1024,  # 100MB内存缓存
                    max_disk_size=500 * 1024 * 1024,    # 500MB磁盘缓存
                    cache_dir=os.path.join(self.temp_dir, "cache")
                )
                logger.info("💾 缓存管理器已初始化（内存:100MB, 磁盘:500MB）")
            except Exception as e:
        ...
        if RESOURCE_MONITOR_AVAILABLE:
            try:
                ...
                logger.warning(f"资源监控器初始化失败: {e}")

        # 🆕 初始化组件卸载管理器（如果可用）
        self.unload_manager = None
        if UNLOAD_MANAGER_AVAILABLE:
            try:
                self.unload_manager = UnloadManager()
                logger.info("🗑️ 组件卸载管理器已初始化")
            except Exception as e:
                logger.warning(f"组件卸载管理器初始化失败: {e}")
    ...
    def cache_result(self, cache_key: str, result: Any, size: int = None):
        """
        缓存处理结果
        ...
        try:
            self.cache_manager.put(cache_key, result, size)
    ...
    def optimize_memory(self):
        ...
        try:
            ...
            if self.unload_manager:
                candidates = self.unload_manager.get_unload_candidates(count=3)
                for component in candidates:
                    self.unload_manager.unload_component(component)
                logger.info(f"🗑️ 已卸载 {len(candidates)} 个非关键组件")
