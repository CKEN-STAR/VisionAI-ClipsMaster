...
class AIViralTransformer:
        ...
        try:
            ...
            for segment in segments:
                try:
                    ...
                    # 调用AI模型
                    ai_response = llm.generate(prompt, temperature=0.8, max_length=256, json_only=True)
    ...
    def _build_reconstruction_prompt(self, segment: Dict[str, Any],
        ...
        try:
            ...
            if language == "zh":
                base_prompt = f"""
请将以下字幕文本重构为更具病毒传播潜力的爆款风格，并仅以JSON对象形式返回：
...
输出格式要求（严格）：
- 仅输出一个合法JSON对象，禁止Markdown代码块、解释或其他额外文本
- 字段：{"text": "重构后的文本"}
"""
            else:
                base_prompt = f"""
Please reconstruct the following subtitle text into a more viral and engaging style, and return ONLY a JSON object:
...
- Return ONLY one valid JSON object; no markdown fences, no explanations
- Fields: {"text": "reconstructed text"}
"""
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    m = re.search(r"\{[\s\S]*?\}", cleaned_response)
                    if m:
                        try:
                ...
                if isinstance(parsed, dict):
                    text_val = parsed.get("text") or parsed.get("reconstructed_text") or ""
                    ...
                    for item in parsed:
                        if isinstance(item, dict):
                            v = item.get("text") or item.get("reconstructed_text")
                            if isinstance(v, str):
                                candidates.append(v)
    ...
    def _optimize_viral_potential(self, segments: List[Dict[str, Any]], language: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, segment in enumerate(optimized_segments):
                ...
                # 如果没有明确的结束时间，尝试从其他字段获取
                if original_end == 0 or original_end <= original_start:
                    # 尝试从duration计算
                    if "duration" in segment:
                        original_end = original_start + segment["duration"]
                    # 尝试从下一个片段的开始时间推断
                    elif i + 1 < len(optimized_segments):
                        next_start = optimized_segments[i + 1].get("start_time", 0)
                        if next_start > original_start:
                            original_end = next_start
                    # 默认给3秒时长
                    else:
                        original_end = original_start + 3.0

                segment_duration = original_end - original_start

                # 确保时长有效
                if segment_duration <= 0:
                    logger.warning(f"片段{i}时长无效: {segment_duration}秒，设置为3秒")
                    segment_duration = 3.0
                    original_end = original_start + segment_duration

                # 保存原始时间码（用于视频切割）
                segment["original_start_time"] = original_start
                segment["original_end_time"] = original_end

                # 设置连续的输出时间码（用于最终视频时间轴）
                segment["start_time"] = continuous_time
                segment["end_time"] = continuous_time + segment_duration
                segment["duration"] = segment_duration

                # 添加时间码重映射标记
                segment["timecode_remapped"] = True

                logger.debug(f"片段{i}: 原始[{original_start:.2f}-{original_end:.2f}] → 输出[{continuous_time:.2f}-{continuous_time + segment_duration:.2f}]")

                # 更新连续时间
                continuous_time += segment_duration

            logger.info(f"时间码重映射完成: {len(optimized_segments)}个片段，总时长{continuous_time:.2f}秒")
            return optimized_segments
