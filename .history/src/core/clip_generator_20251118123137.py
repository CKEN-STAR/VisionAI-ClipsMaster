...
class ClipGenerator:
    ...
    def _process_segments(self, segments: List[Dict[str, Any]],
                         output_path: str, temp_dir: str) -> Dict[str, Any]:
        """处理视频片段，切割并拼接"""
        segment_files = []
        ...
        try:
            ...
            for segment in segments:
                # 🔧 修复：优先使用 original_start_time/original_end_time（AI转换器生成的原始视频时间码）
                # 这些是用于从原视频切割的绝对时间，而 start_time/end_time 是新视频的相对时间
                if 'original_start_time' in segment and 'original_end_time' in segment:
                    # AI转换器生成的片段，使用原始视频时间码进行切割
                    start_time = float(segment['original_start_time'])
                    end_time = float(segment['original_end_time'])
                    logger.debug(f"使用AI转换器的原始时间码: {start_time:.3f}s -> {end_time:.3f}s")
                elif 'original_start' in segment and 'original_end' in segment:
                    ...
                    logger.debug(f"使用SRT格式时间码: {segment['original_start']} -> {segment['original_end']}")
