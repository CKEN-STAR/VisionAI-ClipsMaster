...
class ClipGenerator:
    ...
    def _process_segments(self, video_path: str, segments: List[Dict[str, Any]],
        ...
        def _merge_and_filter_segments(items: List[Dict[str, Any]],
                                       min_dur: float,
                                       gap_thres: float,
                                       max_merged: float,
                                       enforce_order: bool = True) -> List[Dict[str, Any]]:
            """
            对相邻片段做归一化与合并：
            - 优先保证时间顺序
            - 将过短片段与相邻片段按间隙阈值合并
            - 控制合并后最大时长
            返回形如 [{'segment': seg, 'start': s, 'end': e}, ...]
            """
            if not items:
                return []
            segs = items[:]
            ...
            for cur in segs:
                ...
                can_merge = gap <= gap_thres and (max(prev['end'], cur['end']) - prev['start']) <= max_merged
                # 如果上一个片段过短，优先合并
                if can_merge or prev_dur < min_dur or cur_dur < min_dur:
                    # 合并到 prev
                    prev['end'] = max(prev['end'], cur['end'])
                    # 尽量维护 segment 内的原始字段（original_* / start_time / end_time）
                    seg_obj = prev['segment']
                    cur_obj = cur['segment']
                    # original_* 优先
                    if 'original_start' in seg_obj or 'original_start' in cur_obj:
                        seg_obj['original_start'] = seg_obj.get('original_start', cur_obj.get('original_start'))
                        seg_obj['original_end'] = cur_obj.get('original_end', seg_obj.get('original_end'))
                    # 兼容 start_time / end_time
                    if 'start_time' in seg_obj or 'start_time' in cur_obj:
                        seg_obj['start_time'] = seg_obj.get('start_time', cur_obj.get('start_time', prev['start']))
                        seg_obj['end_time'] = cur_obj.get('end_time', prev['end'])
                    # 合并 text（如存在）
                    if 'text' in seg_obj and 'text' in cur_obj:
                        try:
                            seg_obj['text'] = f"{seg_obj.get('text','').strip()} {cur_obj.get('text','').strip()}".strip()
                        except Exception:
                            pass
                    merged[-1] = prev
                else:
            ...
            # 二次过滤：仍低于最小时长的尝试向右合并，否则丢弃
            cleaned: List[Dict[str, Any]] = []
            for i, itm in enumerate(merged):
                dur = max(0.0, itm['end'] - itm['start'])
                if dur >= min_dur:
                    cleaned.append(itm)
                else:
                    # 尝试与下一个合并（不超过上限）
                    if i + 1 < len(merged):
                        nxt = merged[i+1]
                        gap2 = max(0.0, nxt['start'] - itm['end'])
                        if gap2 <= gap_thres and (max(nxt['end'], itm['end']) - itm['start']) <= max_merged:
                            # 合并到下一个
        ...
        for i, itm in enumerate(merged):
            ...
            if dur >= min_dur:
                ...
                if i + 1 < len(merged):
                    ...
                    if gap2 <= gap_thres and (max(nxt['end'], itm['end']) - itm['start']) <= max_merged:
                        # 合并到下一个
                        nxt['start'] = min(nxt['start'], itm['start'])
                        # 同步 segment 原始字段
                        nxt_obj = nxt['segment']
                        cur_obj = itm['segment']
                        if 'original_start' in nxt_obj or 'original_start' in cur_obj:
                            nxt_obj['original_start'] = cur_obj.get('original_start', nxt_obj.get('original_start'))
                            nxt_obj['original_end'] = nxt_obj.get('original_end', cur_obj.get('original_end'))
                        if 'start_time' in nxt_obj or 'start_time' in cur_obj:
                            nxt_obj['start_time'] = cur_obj.get('start_time', nxt['start'])
                            nxt_obj['end_time'] = nxt_obj.get('end_time', nxt['end'])
                        if 'text' in nxt_obj and 'text' in cur_obj:
                            try:
                                nxt_obj['text'] = f"{cur_obj.get('text','').strip()} {nxt_obj.get('text','').strip()}".strip()
                            except Exception:
                                pass
                        # 标记下一个为已更新
                        merged[i+1] = nxt
                    else:
                        logging.getLogger('clip_generator').warning(
                            f"丢弃过短片段(无法合并): start={itm['start']:.2f}, end={itm['end']:.2f}, dur={dur:.2f}s")
                else:
                    logging.getLogger('clip_generator').warning(
                        f"丢弃末尾过短片段: start={itm['start']:.2f}, end={itm['end']:.2f}, dur={dur:.2f}s")
        # 清理排序
        if enforce_order:
            cleaned = sorted([c for c in merged if max(0.0, c['end'] - c['start']) >= min_dur], key=lambda x: (x['start'], x['end']))
        else:
            cleaned = [c for c in merged if max(0.0, c['end'] - c['start']) >= min_dur]
        return cleaned
                        ...
                        logging.getLogger('clip_generator').warning(
                            f"丢弃末尾过短片段: start={itm['start']:.2f}, end={itm['end']:.2f}, dur={dur:.2f}s")
            # 清理被合并后可能仍不达标的（例如合并到了下一个但依旧很短的下一项会进入 cleaned）
            for itm in merged:
                dur = max(0.0, itm['end'] - itm['start'])
                if dur >= min_dur and (not cleaned or cleaned[-1] is not itm):
                    # 避免重复加入
                    pass
            # 清理排序
            if enforce_order:
