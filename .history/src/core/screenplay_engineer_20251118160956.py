...
class ScreenplayEngineer:
    ...
    def load_subtitles(self, srt_data) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(f"用户提示: {friendly_error.message}")
            return []
    ...
    def reconstruct_screenplay(self, srt_input=None, analysis: Optional[Dict] = None,
                              language: Optional[str] = None, target_style: str = "viral"):
        """
        重构剧本为爆款风格 - 核心功能实现（支持多种调用方式）

        Args:
            srt_input: SRT文件路径、SRT内容字符串或字幕列表，如果为None则使用当前加载的字幕
            analysis: 剧情分析结果（可选，用于workflow_manager调用）
            language: 语言代码（可选，用于workflow_manager调用）
            target_style: 目标风格，默认为"viral"（爆款）

        Returns:
            List[Dict]: 标准化的重构后字幕列表，每个元素包含 start, end, text, duration 字段
        """
        # 🆕 兼容workflow_manager的调用方式: reconstruct_screenplay(subtitles, analysis, language)
        if srt_input is not None and isinstance(srt_input, list) and analysis is not None and language is not None:
            logger.info(f"🎭 使用工作流模式重构剧本，语言: {language}")
            return self.reconstruct_screenplay_workflow(srt_input, analysis, language)
        ...
        try:
            ...
            # 5. 转换为标准化格式（✅ 关键修复：保留原始时间码）
            standardized_segments = []
            timeline_segments = new_timeline.get("segments", [])

            for i, timeline_seg in enumerate(timeline_segments):
                # 确保每个片段都有标准化的字段
                standardized_segment = {
                    # 新时间轴（用于爆款SRT显示）
                    "start": float(timeline_seg.get("start_time", 0)),
                    "end": float(timeline_seg.get("end_time", 0)),
                    "text": str(timeline_seg.get("text", "")),
                    "duration": float(timeline_seg.get("duration", 0)),

                    # ✅ 保留原始时间码（用于视频切割）
                    "original_start": float(timeline_seg.get("original_start", 0)),
                    "original_end": float(timeline_seg.get("original_end", 0)),
                    "original_duration": float(timeline_seg.get("original_duration", 0)),

                    # 其他元数据
                    "type": timeline_seg.get("type", ""),
                    "priority": timeline_seg.get("priority", 0.5),
                    "sequence_index": timeline_seg.get("sequence_index", i)
                }
            ...
            logger.info(f"✅ 每个片段包含：新时间轴（用于SRT）+ 原始时间码（用于视频切割）")
            ...
            logger.info(f"💡 用户提示: {friendly_error.message}")
            logger.info(f"💡 建议: {', '.join(friendly_error.solutions)}")
            # 返回原始字幕的标准化格式作为回退
            fallback_segments = []
        ...
        try:
            ...
            for seg_type, type_segments in segments_by_type.items():
                ...
                if seg_type == "opening":
                    ...
                    # 高潮：选择评分最高的“连续窗口”，保持时间连续性
                    # 已按 sequence_index 排好序，取窗口大小 w（<=4）的最佳窗口
                    w = min(4, len(type_segments))
                    best_i, best_score = 0, -1e9
                    for i in range(0, len(type_segments) - w + 1):
                        window = type_segments[i:i+w]
                        score = sum(s.get("final_score", 0.0) for s in window)
                        # 邻接奖励：相邻 sequence_index 差为1 则加分，鼓励连续
                        adj_bonus = 0.0
                        for j in range(len(window)-1):
                            if window[j+1].get("sequence_index", 0) - window[j].get("sequence_index", 0) == 1:
                                adj_bonus += 0.1
                        score += adj_bonus
                        if score > best_score:
                            best_score, best_i = score, i
                    selected_segments.extend(type_segments[best_i:best_i+w])
            ...
            if len(unique_segments) < target_min_count:
                ...
                # 减少片段但保持连贯性：采用定长滑窗选择最高得分的时间连续窗口
                unique_segments.sort(key=lambda x: x.get("sequence_index", 0))
                w = target_max_count
                best_i, best_score = 0, -1e9
                scores = [s.get("final_score", 0.0) for s in unique_segments]
                for i in range(0, len(unique_segments) - w + 1):
                    window_scores = scores[i:i+w]
                    score = sum(window_scores)
                    # 邻接连续性加成
                    adj_bonus = 0.0
                    for j in range(i, i+w-1):
                        if unique_segments[j+1].get("sequence_index", 0) - unique_segments[j].get("sequence_index", 0) == 1:
                            adj_bonus += 0.05
                    score += adj_bonus
                    if score > best_score:
                        best_score, best_i = score, i
                unique_segments = unique_segments[best_i:best_i+w]
    ...
    def _generate_new_timeline(self, segments: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        关键修复：
        1. 保留原始视频时间码（original_start/original_end）用于视频切割
        2. 生成新的连续时间轴（start_time/end_time）用于爆款SRT显示
        3. 两个时间轴分开管理，各司其职
        """
        try:
            if not segments:
            ...
            for i, segment in enumerate(segments):
                ...
                new_end_time = current_time + segment_duration

                # ✅ 关键修复：保留原始视频时间码
                original_start = segment.get("start_time", 0)
                original_end = segment.get("end_time", 0)
                ...
                new_segment = {
                    ...
                    "sequence_index": segment.get("sequence_index", i)
                }
            ...
            logger.info(f"✅ 已保留 {len(timeline_segments)} 个片段的原始视频时间码")
            return timeline
    ...
    def reconstruct_plot(self, original_plot: str, language: str = "zh") -> str:
        ...
        try:
            ...
            # 第一步：深度语义分析
            logger.info("📊 步骤1/7：深度语义分析")
            semantic_analysis = self._deep_semantic_analysis(original_plot, language)
            logger.info(f"✅ 语义分析完成: 主导情感={semantic_analysis.get('dominant_emotion', 'unknown')}, 情感强度={semantic_analysis.get('emotional_intensity', 0):.2f}")

            # 第二步：剧情结构解析
            logger.info("📖 步骤2/7：叙事结构解析")
            narrative_structure = self._analyze_narrative_structure(original_plot, language)
            logger.info(f"✅ 叙事结构分析完成: 结构完整性={narrative_structure.get('structure_completeness', 0):.2f}, 故事弧强度={narrative_structure.get('story_arc_strength', 0):.2f}")

            # 第三步：角色关系识别
            logger.info("👥 步骤3/7：角色关系识别")
            character_relations = self._identify_character_relations(original_plot, language)
            logger.info(f"✅ 角色关系识别完成: {len(character_relations)} 个关系")

            # 第四步：情节转折点检测
            logger.info("🎯 步骤4/7：情节转折点检测")
            plot_turning_points = self._detect_plot_turning_points(original_plot, language)
            logger.info(f"✅ 检测到 {len(plot_turning_points)} 个转折点")

            # 第五步：应用原片→爆款转换算法
            logger.info("🚀 步骤5/7：爆款化改造")
            ...
            logger.info(f"✅ 爆款化改造完成: {len(original_plot)} -> {len(viral_transformation)} 字符")

            # 第六步：智能时间轴重新分配
            logger.info("⏱️ 步骤6/7：智能时间轴重新分配")
            reconstructed_plot = self._intelligent_timeline_reallocation(
                viral_transformation, language
            )
            logger.info(f"✅ 时间轴重新分配完成: {len(viral_transformation)} -> {len(reconstructed_plot)} 字符")

            # 第七步：质量验证和优化
            logger.info("✔️ 步骤7/7：质量验证和优化")
            final_plot = self._validate_and_optimize_reconstruction(
                original_plot, reconstructed_plot, language
            )
            quality_score = self._calculate_reconstruction_quality(original_plot, final_plot)
            logger.info(f"✅ 质量验证完成: 评分={quality_score:.2f}/10")

            logger.info(f"🎉 剧情重构完成: {len(original_plot)} -> {len(final_plot)} 字符, 质量评分: {quality_score:.2f}/10")
    ...
    def _distribute_reconstructed_text(self, original_subtitles: List[Dict[str, Any]],
                                       reconstructed_text: str,
                                       language: str) -> List[Dict[str, Any]]:
        """
        将重构后的文本分配到“语义完句”级别的片段上：
        - 先按原始字幕的语义边界做邻接拼接（同一段话内的多条原始字幕合为一组）
        - 再将重构后的句子一一映射到这些“分配单元”，并继承首尾时间，彻底避免“话未完就切”
        """
        try:
            import re
            ...
            def _is_incomplete(text: str) -> bool:
                ...
                if not t:
                    return True
                # 省略号、逗号、顿号、破折号、分号、冒号视为未完
                if t.endswith(("…", "...", "，", "、", "——", "—", "；", ";", "：", ":")):
                    return True
                # 引号未闭合视为未完
                if t.count('“') > t.count('”') or t.count('‘') > t.count('’') or t.count('"') % 2 == 1 or t.count("'") % 2 == 1:
                    return True
                # 终止标点（含英文），且可带右引号/右括
                if re.search(r"[。！？.!?][”’】））〉》』」]?\s*$", t):
                    return False
            ...
            # 1) 将原始字幕按语义边界分组（同一段完整话语为一组）
            max_chain = 3
            gap_limit = 2.0  # 秒
            units: List[Dict[str, Any]] = []
            cur = None

            for i, seg in enumerate(original_subtitles):
                ...
                if cur is None:
                    ...
                    # 与上一条的间隙
                    gap = max(0.0, st - float(cur["end_time"]))
                    if cur["count"] >= max_chain or gap > gap_limit or not _is_incomplete(cur["texts"][-1]):
                        # 结束当前组
                        cur_text = "".join(cur["texts"]) if language == "zh" else " ".join(cur["texts"])  # 中文不加空格
                        units.append({
            ...
            logger.info(f"[Group] 原始条目={len(original_subtitles)} → 分配单元={len(units)} (max_chain={max_chain}, gap≤{gap_limit}s)")

            # 2) 将重构文本切分为句子（保留终止标点与右引号）
            def _split_sentences(text: str, lang: str) -> List[str]:
                ...
                if lang == "zh":
                    ...
                    pats = re.findall(r".*?[.!?](?:[\"')\]]+)?|.+", s)
                # 清理空白
                out = [p.strip() for p in pats if p and p.strip()]
                return out

            sentences = _split_sentences(reconstructed_text, language)

            # 3) 若句子数不足分配单元，尝试在逗号/顿号处分裂长句；仍不足则最后回填
            def _try_split_once(x: str) -> List[str]:
                # 优先中文逗号/顿号，其次英文逗号/分号/冒号/破折号
                for sep in ["，", "、", ",", ";", ":", "——", "—"]:
                    if sep in x and len(x) >= 12:
                        mid = len(x) // 2
                        # 找离中点最近的分隔符
                        idxs = [i for i, ch in enumerate(x) if ch == sep]
                        if idxs:
                            k = min(idxs, key=lambda i: abs(i - mid))
                            left, right = x[:k+1].strip(), x[k+1:].strip()
                            if len(left) >= 6 and len(right) >= 6:
                                return [left, right]
                return [x]

            while len(sentences) < len(units):
                # 找最长的句子尝试一分为二
                if not sentences:
                    break
                longest_idx = max(range(len(sentences)), key=lambda i: len(sentences[i]))
                repl = _try_split_once(sentences[longest_idx])
                if len(repl) == 1:
                    # 无法再分，退出
                    break
                # 用拆分结果替换
                sentences = sentences[:longest_idx] + repl + sentences[longest_idx+1:]

            if len(sentences) < len(units):
                # 兜底：重复最后一句直至对齐（极少触发）
                last = sentences[-1] if sentences else ""
                sentences = sentences + [last] * (len(units) - len(sentences))

            # 4) 一一映射生成新的片段（直接使用组的首尾时间，根除截断）
            screenplay: List[Dict[str, Any]] = []
            for i, u in enumerate(units):
                seg = {
                    "start_time": float(u.get("start_time", 0) or 0.0),
                    "end_time": float(u.get("end_time", 0) or 0.0),
                    "text": sentences[i] if i < len(sentences) else u.get("text", "")
                }
                # 情感标签（可选）
                try:
                    seg["sentiment"] = self._analyze_sentence_sentiment(seg["text"], language)
                except Exception:
                    pass
                screenplay.append(seg)

            return screenplay
        except Exception as e:
            logger.error(f"分配重构文本失败: {e}")
            return original_subtitles

    def _load_coherence_config(self) -> Dict[str, Any]:
        """从配置读取叙事连贯性参数，带默认值与容错"""
        defaults = {"enable_bridging": True, "gap_threshold": 2.0}
        try:
            import os, json
            cfg_path = os.path.join("configs", "quality_probes.json")
            if os.path.exists(cfg_path):
                ...
                if isinstance(coh, dict):
                    return {
                        "enable_bridging": bool(coh.get("enable_bridging", defaults["enable_bridging"])),
                        "gap_threshold": float(coh.get("gap_threshold", defaults["gap_threshold"]))
                    }
        except Exception:
            pass
        return defaults

    def _postprocess_screenplay_for_coherence(self, segments: List[Dict[str, Any]], language: str,
                                              gap_threshold: float = 2.0,
                                              enable_bridging: bool = True) -> List[Dict[str, Any]]:
        """
        文本连贯性后处理（非侵入式）：
        - 在不改变原始切割时间(start_time/end_time)的前提下，按拼接顺序计算输出时间轴(output_start/output_end)
        - 对原始时间轴相邻片段存在较大间隙时，为后片段文本注入桥接短语（不新增片段，不改变时长）
        """
        try:
            if not segments:
                return segments

            # 1) 桥接短语注入（基于原始相邻片段时间缝隙）
            zh_connectors = ["与此同时，", "随后，", "转眼间，", "这时，", "接着，"]
            en_connectors = ["Meanwhile, ", "Soon after, ", "Suddenly, ", "At that moment, ", "Then, "]
            bridge_list = zh_connectors if language == "zh" else en_connectors

            enhanced: List[Dict[str, Any]] = []
            for i, seg in enumerate(segments):
                seg = dict(seg)  # 浅拷贝，避免修改入参
                if i > 0:
                    prev = segments[i - 1]
                    prev_end = float(prev.get("end_time", 0) or 0)
                    curr_start = float(seg.get("start_time", 0) or 0)
                    gap = max(0.0, curr_start - prev_end)
                    if enable_bridging and gap > gap_threshold:
                        # 若文本未以常见连接词开头，则注入一个
                        if seg.get("text"):
                            if not any(str(seg["text"]).startswith(pref) for pref in bridge_list):
                                # 选择一个稳定的连接词（与索引相关，避免随机）
                                prefix = bridge_list[i % len(bridge_list)]
                                seg["text"] = f"{prefix}{seg['text']}"
                enhanced.append(seg)

            # 2) 输出时间轴线性化：根据各段原始时长，在输出视频的时间轴上顺序累加
            t = 0.0
            for seg in enhanced:
                start_src = float(seg.get("start_time", 0) or 0)
                end_src = float(seg.get("end_time", 0) or 0)
                duration = max(0.0, end_src - start_src)
                seg["output_start"] = t
                seg["output_end"] = t + duration
                t += duration
    ...
    def _analyze_sentence_sentiment(self, sentence: str, language: str) -> Dict[str, Any]:
        """
        分析句子情感
        ...
        emotion_keywords = {
            "zh": {
                "positive": ["开心", "快乐", "幸福", "成功", "胜利", "美好", "惊喜", "满足", "爱", "喜欢"],
                "negative": ["痛苦", "悲伤", "失败", "绝望", "愤怒", "恐惧", "焦虑", "失望", "孤独", "讨厌"],
                "intense": ["震撼", "惊人", "不可思议", "令人震惊", "史无前例", "惊天动地", "太", "超级"]
            },
            "en": {
                "positive": ["happy", "joy", "success", "victory", "beautiful", "love", "like", "great", "amazing"],
                "negative": ["pain", "sad", "failure", "despair", "anger", "fear", "anxiety", "disappointed", "hate"],
                "intense": ["shocking", "amazing", "incredible", "stunning", "unprecedented", "very", "super"]
            }
        }

        keywords = emotion_keywords.get(language, emotion_keywords["zh"])
        sentence_lower = sentence.lower()

        # 计算情感分数
        positive_count = sum(1 for word in keywords["positive"] if word in sentence_lower)
        negative_count = sum(1 for word in keywords["negative"] if word in sentence_lower)
        intense_count = sum(1 for word in keywords["intense"] if word in sentence_lower)

        # 确定主导情感
        if positive_count > negative_count:
            label = "POSITIVE"
            intensity = min(0.5 + positive_count * 0.1 + intense_count * 0.1, 1.0)
        elif negative_count > positive_count:
            label = "NEGATIVE"
            intensity = min(0.5 + negative_count * 0.1 + intense_count * 0.1, 1.0)
        else:
            label = "NEUTRAL"
            intensity = 0.5 + intense_count * 0.1
    ...
    def generate_screenplay(self, original_subtitles: List[Dict[str, Any]],
        ...
        try:
            logger.info(f"🎬 开始生成混剪剧本，使用7步重构算法")

            # 自动检测语言
            if language == "auto":
                full_text = " ".join([sub.get("text", "") for sub in original_subtitles])
                language = "zh" if any('\u4e00' <= char <= '\u9fff' for char in full_text) else "en"
                logger.info(f"自动检测语言: {language}")

            # 合并所有字幕文本用于重构
            full_plot = " ".join([sub.get("text", "") for sub in original_subtitles])

            # 🆕 使用7步重构算法生成爆款剧本
            reconstructed_plot = self.reconstruct_plot(full_plot, language)

            # 将重构后的文本分配回字幕段
            screenplay = self._distribute_reconstructed_text(
                original_subtitles,
                reconstructed_plot,
                language
            )
            # 叙事连贯性后处理：桥接短语注入 + 输出时间轴线性化（不改变原始切割时间）
            _coh = self._load_coherence_config()
            screenplay = self._postprocess_screenplay_for_coherence(
                screenplay,
                language,
                gap_threshold=float(_coh.get("gap_threshold", 2.0)),
                enable_bridging=bool(_coh.get("enable_bridging", True))
            )
    ...
    def export_srt(self, segments: List[Dict[str, Any]], output_path: str) -> bool:
        ...
        🔧 关键修复：保留原始视频时间码信息
        - 新时间轴（start/end）用于SRT显示
        - 原始时间码（original_start/original_end）作为注释保存，供视频切割使用
        """
        try:
            with open(output_path, 'w', encoding='utf-8') as f:
                for i, segment in enumerate(segments):
                    # 🔧 修复：优先使用标准化的字段名
                    # 新时间轴（用于SRT显示）
                    start_time = segment.get("start", segment.get("start_time", segment.get("output_start", 0)))
                    end_time = segment.get("end", segment.get("end_time", segment.get("output_end", 0)))
                    text = segment.get("text", "")

                    # 转换时间格式
                    start_formatted = self._seconds_to_srt_time(float(start_time))
                    end_formatted = self._seconds_to_srt_time(float(end_time))

                    # 写入SRT格式
                    f.write(f"{i+1}\n")
                    f.write(f"{start_formatted} --> {end_formatted}\n")

                    # 🔧 关键修复：如果存在原始时间码，作为注释保存
                    # 这样ClipGenerator可以读取并使用正确的时间码进行视频切割
                    if 'original_start' in segment and 'original_end' in segment:
                        orig_start = float(segment['original_start'])
                        orig_end = float(segment['original_end'])
                        orig_start_fmt = self._seconds_to_srt_time(orig_start)
                        orig_end_fmt = self._seconds_to_srt_time(orig_end)
                        f.write(f"# ORIGINAL_TIME: {orig_start_fmt} --> {orig_end_fmt}\n")
                        f.write(f"# ORIGINAL_SECONDS: {orig_start:.3f} --> {orig_end:.3f}\n")

                    f.write(f"{text}\n\n")

            logger.info(f"✅ 成功导出SRT文件: {output_path}")
            logger.info(f"✅ 包含 {len(segments)} 个片段，{sum(1 for s in segments if 'original_start' in s)} 个片段保留了原始时间码")
            return True
        ...
        try:
            ...
            for block in blocks:
                ...
                try:
                    ...
                    for line in lines[2:]:
                        if line.startswith('# ORIGINAL_SECONDS:'):
                            # 解析原始时间码（秒格式）
                            orig_match = re.search(r'(\d+\.\d+) --> (\d+\.\d+)', line)
                            if orig_match:
                                original_start = float(orig_match.group(1))
                                original_end = float(orig_match.group(2))
                        elif line.startswith('# ORIGINAL_TIME:'):
                            # 跳过时间格式的注释（已有秒格式）
                            continue
                    ...
                    subtitle = {
                        ...
                        "start": start_seconds,  # 标准化字段名
                        "end": end_seconds,      # 标准化字段名
                        "duration": end_seconds - start_seconds,
                        "text": text
                    }

                    # 🔧 关键修复：如果存在原始时间码，添加到字幕对象
                    if original_start is not None and original_end is not None:
                        subtitle["original_start"] = original_start
                        subtitle["original_end"] = original_end
                        subtitle["original_duration"] = original_end - original_start

                    subtitles.append(subtitle)

                except Exception as e:
                    logger.warning(f"解析字幕块失败: {e}, 块内容: {block[:100]}...")
                    continue

            logger.info(f"✅ 成功导入SRT文件: {srt_path}")
            logger.info(f"✅ 共 {len(subtitles)} 个片段，{sum(1 for s in subtitles if 'original_start' in s)} 个片段包含原始时间码")
            return subtitles
    ...
    def reconstruct_screenplay_workflow(self, subtitles: List[Dict], analysis: Dict, language: str) -> List[Dict]:
        ...
        """
        try:
            logger.info(f"🎭 开始剧本重构（7步算法），语言: {language}, 字幕数: {len(subtitles)}")

            # 合并所有字幕文本
            full_plot = " ".join([sub.get("text", "") for sub in subtitles])

            # 🆕 使用7步重构算法
            logger.info(f"📝 原始剧本长度: {len(full_plot)} 字符")
            reconstructed_plot = self.reconstruct_plot(full_plot, language)
            logger.info(f"✨ 重构后剧本长度: {len(reconstructed_plot)} 字符")

            # 将重构后的文本分配回字幕段
            reconstructed_subtitles = self._distribute_reconstructed_text(
                subtitles,
                reconstructed_plot,
                language
            )
