...
class ClipGenerator:
    ...
    def __init__(self):
        ...
        # 使用FFmpegUtils获取FFmpeg路径
        self.ffmpeg_path = ffmpeg_utils.get_ffmpeg_path()
        self.ffprobe_path = ffmpeg_utils.get_ffprobe_path()

        # 检查FFmpeg可用性
        if not ffmpeg_utils.is_available():
            logger.warning(f"FFmpeg不可用，路径: {self.ffmpeg_path}")
        else:
            logger.info(f"FFmpeg已就绪，路径: {self.ffmpeg_path}")


    def check_ffmpeg_availability(self):
        """检查FFmpeg可用性"""
        return ffmpeg_utils.is_available()
        ...
        try:
            if not os.path.exists(video_path):
                ...
                return {
                    ...
                    'format': 'unknown',
                    'error': error_msg,
                    'user_message': '找不到指定的视频文件，请检查文件路径是否正确。'
                }
            ...
            error_msg = f"获取视频信息超时: {video_path}"
            ...
            return {
                ...
                'user_message': '视频文件分析超时，可能文件过大或格式复杂。将使用默认参数处理。'
            }
