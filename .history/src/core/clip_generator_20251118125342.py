    ...
    def _process_segments(self, video_path: str, segments: List[Dict[str, Any]],
        ...
        try:
            ...
            for segment in segments:
                # 🔧 修复：优先使用重映射后的原始时间码（用于视频切割）
                # 如果AI转换器已经重映射时间码，使用 original_start_time/original_end_time
                if segment.get("timecode_remapped") and 'original_start_time' in segment and 'original_end_time' in segment:
                    # 使用AI转换器保存的原始时间码（用于从原视频切割）
                    start_time = float(segment['original_start_time'])
                    end_time = float(segment['original_end_time'])
                    logger.debug(f"使用重映射的原始时间码: {start_time:.2f}-{end_time:.2f}")
                # 获取时间信息（优先使用original_start/original_end，然后是original_time，最后是time）
                    ...
                    logger.debug(f"使用原始SRT时间码: {segment['original_start']} -> {segment['original_end']}")
