...
2. 阿里云DashScope (百炼) - https://dashscope.aliyuncs.com
...
class CloudPlatform:
    ...
    DASHSCOPE = "dashscope"      # 阿里云DashScope (百炼)
...
PLATFORM_CONFIG = {
    ...
    CloudPlatform.DASHSCOPE: {
        ...
        "models": {
            ...
            CloudModel.DEEPSEEK_V3_2: "deepseek-v3",  # DashScope兼容DeepSeek-V3
        },
        "headers_template": {
            "Content-Type": "application/json",
            "Authorization": "Bearer {api_key}"
...
class DashScopeProvider(BaseCloudProvider):
    ...
    def chat_completion(self, messages: List[Dict], model: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"阿里云DashScope API请求失败: {e}")
            raise
...
def get_supported_platforms() -> List[Dict[str, str]]:
    ...
    return [
        ...
        {"id": CloudPlatform.DASHSCOPE, "name": "阿里云DashScope", "url": "https://dashscope.aliyuncs.com"}
    ]
