...
# GPU相关导入（在部分环境中，torch 可能因 DLL 依赖导致 OSError，需要更宽松的兜底）
try:
    import torch
    import torchvision.transforms as transforms
    TORCH_AVAILABLE = True
except Exception:
    # 广义捕获：包括 ImportError/OSError/DLL 加载失败等
    TORCH_AVAILABLE = False
    class _TorchStub:
        class device:  # 仅用于类型注解占位，避免函数定义期 NameError
            pass
        class Tensor:  # 类型注解占位
            pass
    torch = _TorchStub()  # 提供 torch.device/Tensor 可访问占位
    transforms = None
