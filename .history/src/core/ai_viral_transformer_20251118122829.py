...
class AIViralTransformer:
    ...
    def _identify_key_segments(self, subtitles: List[Dict[str, Any]],
        ...
        Returns:
            List[Dict[str, Any]]: 关键片段列表（带重新计算的连续时间轴）
        """
        try:
            key_segments = []
            ...
            # 构建关键片段并重新计算时间轴
            current_time = 0.0  # 新视频的累积时间

            for index in sorted(key_indices):
                if index < len(subtitles):
                    original_segment = subtitles[index]
                    segment = original_segment.copy()
                    segment['source_video_path'] = original_segment.get('source_video_path', '') # 保留来源视频路径

                    # 保存原始时间码（用于视频切割）
                    segment["original_start_time"] = original_segment.get("start_time", 0)
                    segment["original_end_time"] = original_segment.get("end_time", 0)

                    # 计算原始片段的时长
                    original_duration = segment["original_end_time"] - segment["original_start_time"]

                    # 重新计算新视频中的时间轴（连续的）
                    segment["start_time"] = current_time
                    segment["end_time"] = current_time + original_duration
            ...
            logger.info(f"识别出{len(key_segments)}个关键片段，总时长: {current_time:.2f}秒")
            logger.info(f"时间轴已重新计算为连续序列")
            return key_segments
