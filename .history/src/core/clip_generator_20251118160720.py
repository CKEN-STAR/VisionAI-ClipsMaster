    ...
    def _process_segments(self, video_path: str, segments: List[Dict[str, Any]],
        ...
        try:
            ...
            for segment in segments:
                ...
                if 'original_start' in segment and 'original_end' in segment:
                    ...
                    if isinstance(original_start, (int, float)):
                        # 浮点数格式（秒）
                        start_time = float(original_start)
                        end_time = float(original_end)
                        logger.debug(f"使用原始时间码（浮点数）: {start_time:.2f}s -> {end_time:.2f}s")
                    else:
                        # SRT字符串格式（HH:MM:SS,mmm）
                        start_time = self._parse_srt_time(original_start)
                        end_time = self._parse_srt_time(original_end)
                        logger.debug(f"使用原始时间码（SRT格式）: {original_start} -> {original_end}")
            ...
            for i, item in enumerate(valid_segments):
                ...
                # 🔧 关键修复：优先使用原始视频时间码，而不是新时间轴
                # 新时间轴（start/end）仅用于SRT显示，视频切割必须使用原始时间码
                if segment and 'original_start' in segment and 'original_end' in segment:
                    start_time = float(segment['original_start'])
                    end_time = float(segment['original_end'])
                    logger.info(f"[CutPlan] seg#{i:03d} 使用原始时间码: {start_time:.3f}s - {end_time:.3f}s")
                else:
                    # 回退：如果没有原始时间码，使用item中的时间（可能导致不连贯）
                    start_time = item['start']
                    end_time = item['end']
                    logger.warning(f"[CutPlan] seg#{i:03d} ⚠️ 缺少原始时间码，使用新时间轴: {start_time:.3f}s - {end_time:.3f}s")

                # 🔧 修复：头部最小过渡间隙
                # 避免上一段刚结束下一段立刻起（人耳感知为硬切）
                # 但要注意：如果使用了原始时间码，这个调整可能会破坏原始视频的连续性
                try:
                    head_min_gap = float(proc.get('head_transition_min_gap', 0.08))
                    if prev_final_end is not None and (start_time < prev_final_end + head_min_gap):
                        orig_start = start_time

                        # 🔧 关键修复：如果使用了原始时间码，不要调整起点
                        # 因为原始时间码已经是视频中正确的时间点
                        if segment and 'original_start' in segment:
                            logger.debug(f"[CutPlan] seg#{i:03d} 使用原始时间码，跳过起点调整")
                        else:
                            # 只有在使用新时间轴时才调整起点
                    ...
                    logger.warning(f"[CutPlan] seg#{i:03d} 头部间隙调整失败: {e}")
                ...
                try:
                    ...
                    # 🔧 修复：原始结束时间提示（若存在）
                    # original_end 已经是浮点数（秒），不需要再解析
                    try:
                        orig_end_hint = None
                        seg_meta = segment or {}
                        if 'original_end' in seg_meta:
                            # 直接使用浮点数，不需要解析
                            orig_end_hint = float(seg_meta.get('original_end'))
                            logger.debug(f"[CutPlan] seg#{i:03d} 使用原始结束时间提示: {orig_end_hint:.3f}s")
                        elif 'original_time' in seg_meta:
                            orig_end_hint = float(seg_meta.get('original_time', {}).get('end'))
                    except Exception as e:
                        logger.warning(f"[CutPlan] seg#{i:03d} 解析原始结束时间失败: {e}")
                        orig_end_hint = None
                    ...
                    # 🔧 关键修复：若存在原始结束时间提示，优先使用它
                    # 这确保了使用原始视频的实际对话结束时间，避免截断
                    if isinstance(orig_end_hint, (int, float)) and orig_end_hint > 0:
                        # 使用原始结束时间，并添加少量缓冲（0.15秒）确保对话完整
                        target_end = max(target_end, orig_end_hint + 0.15)
                        logger.debug(f"[CutPlan] seg#{i:03d} 使用原始结束时间+缓冲: {target_end:.3f}s")
                    ...
                    #[object Object]增强：记录详细的剪辑计划日志，便于定位边界问题
                    try:
                        ...
                        # 详细日志：显示原始时间码和最终时间码
                        if segment and 'original_start' in segment and 'original_end' in segment:
                            logger.info(f"[CutPlan] seg#{i:03d} 原始视频: {segment['original_start']:.3f}s-{segment['original_end']:.3f}s")
                        logger.info(f"[CutPlan] seg#{i:03d} 最终切割: {start_time:.3f}s-{end_time:.3f}s dur={final_duration:.3f}s")
                        logger.info(f"[CutPlan] seg#{i:03d} 缓冲设置: pad={univ_pad:.2f}s{extra_tag} punct={ends_with_punct}")
                        logger.info(f"[CutPlan] seg#{i:03d} 文本: {segment.get('text', '')[:50]}...")
                    except Exception as e:
                        logger.warning(f"[CutPlan] seg#{i:03d} 日志记录失败: {e}")
                except Exception:
                    pass
                ...
                # --- 故障域隔离：手动验证 --- #
                # 打印出用于手动执行FFmpeg命令的精确值，以隔离问题源。
                # 如果手动执行生成的单个片段文件末尾台词已截断，则问题出在切割阶段。
                # 如果单个片段文件台词完整，则问题出在拼接阶段。
                logger.critical(f"--- MANUAL VERIFICATION --- Segment Index: {i} ---")
                logger.critical(f"FFmpeg Cut Command Values:")
                logger.critical(f"  Input Path: '{video_path}'")
                logger.critical(f"  Output Path: '{segment_file}'")
                logger.critical(f"  Start Time: {start_time:.4f}")
                logger.critical(f"  Duration: {duration:.4f}")
                logger.critical(f"--- END MANUAL VERIFICATION ---")
