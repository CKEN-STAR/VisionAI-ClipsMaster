...
class ScreenplayEngineer:
    ...
    def _postprocess_screenplay_for_coherence(self, segments: List[Dict[str, Any]], language: str,
        ...
        try:
            ...
            t = 0.0
            for seg in enhanced:
                # 🔧 关键修复：优先使用AI转换器保存的原始时间码来计算时长
                if seg.get("timecode_remapped") and 'original_start_time' in seg and 'original_end_time' in seg:
                    start_src = float(seg.get("original_start_time", 0) or 0.0)
                    end_src = float(seg.get("original_end_time", 0) or 0.0)
                    logger.debug(f"Coherence: 使用重映射的原始时间码计算时长: {start_src:.2f}-{end_src:.2f}")
                else:
                    # 回退到使用标准时间码
                    start_src = float(seg.get("start_time", 0) or 0.0)
                    end_src = float(seg.get("end_time", 0) or 0.0)

                duration = max(0.0, end_src - start_src)

                # 将计算出的连续时间码保存到 output_start/output_end
                seg["output_start"] = t
    ...
    def generate_screenplay(self, original_subtitles: List[Dict[str, Any]],
        ...
        try:
            ...
            # 🆕 使用AIViralTransformer进行转换
            from src.core.ai_viral_transformer import AIViralTransformer
            transformer = AIViralTransformer()
            screenplay = transformer.transform_to_viral(
                original_subtitles,
                language=language,
                style=preset_name or "viral",
                intensity=0.8 # 默认强度
            )
