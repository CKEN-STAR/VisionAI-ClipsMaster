    ...
    def _distribute_reconstructed_text(self, original_subtitles: List[Dict[str, Any]],
        ...
        try:
            ...
            if language == "zh":
                ...
                # 英文按句号分句
                sentences = [s.strip() for s in reconstructed_text.split('.') if s.strip()]

            # 过滤空句子
            sentences = [s.strip() for s in sentences if s.strip()]

            # 如果句子数量少于字幕段数量，重复使用
            if len(sentences) < len(original_subtitles):
                sentences = sentences * (len(original_subtitles) // len(sentences) + 1)

            # 分配文本到字幕段
            screenplay = []
            for i, subtitle in enumerate(original_subtitles):
                if i < len(sentences):
                    new_segment = subtitle.copy()
                    new_segment["text"] = sentences[i]

                    # 添加情感标签（基于重构算法的分析）
                    sentiment = self._analyze_sentence_sentiment(sentences[i], language)
                    new_segment["sentiment"] = sentiment

                    screenplay.append(new_segment)
                else:
                    screenplay.append(subtitle)
