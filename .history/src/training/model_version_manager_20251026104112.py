...
class ModelVersionManager:
    ...
    def register_new_version(
        ...
        Args:
            ...
            version_id: 指定版本ID（可选，用于覆盖模式）
                       - None: 自动生成时间戳版本ID（默认）
                       - "current": 固定版本ID，用于持续迭代训练
            overwrite: 是否覆盖现有版本（默认False）
                      - True: 如果版本已存在，覆盖它（用于持续迭代训练）
                      - False: 如果版本已存在，返回错误
        ...
        if existing_version and not overwrite:
            logger.warning(f"⚠️ 版本已存在: {version_id}，使用 overwrite=True 来覆盖")
            return ""
        ...
        try:
            ...
            # 如果是覆盖模式，先删除旧版本
            if existing_version:
                logger.info(f"   覆盖现有版本: {version_id}")
                # 从列表中移除旧版本
            ...
            logger.info(f"✅ 版本注册成功: {version_id} (覆盖={overwrite})")
    ...
    def _remove_version_files(self, version_id: str, delete_actual_files: bool = False):
        ...
        Args:
            version_id: 版本ID
            delete_actual_files: 是否删除实际的模型文件（引用模式下）
        """
        # 查找版本信息
        version_info = None
        for v in self.versions["versions"]:
        ...
        if not version_info:
            logger.warning(f"   未找到版本信息: {version_id}")
            return
    ...
    def delete_version(self, version_id: str, delete_actual_files: bool = True) -> bool:
        """
        删除指定版本（UI调用的方法）

        Args:
            version_id: 版本ID
            delete_actual_files: 是否删除实际的模型文件（默认True，真删除）

        Returns:
            是否成功
        """
        return self.cleanup_version(version_id, delete_actual_files=delete_actual_files)

    def cleanup_version(self, version_id: str, delete_actual_files: bool = True) -> bool:
        """
        手动清理指定版本
        ...
        Returns:
            是否成功
        """
        logger.info(f"🧹 手动清理版本: {version_id} (delete_actual_files={delete_actual_files})")
        ...
        for i, version in enumerate(self.versions["versions"]):
            if version["version_id"] == version_id:
                # 删除文件
                self._remove_version_files(version_id, delete_actual_files=delete_actual_files)
