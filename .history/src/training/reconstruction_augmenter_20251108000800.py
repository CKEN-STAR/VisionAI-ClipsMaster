        ...
        try:
            ...
            for i in range(num_variants):
                try:
                    ...
                    # 计算Step7质量评分（0~10）
                    try:
                        quality_score = float(self.engineer._calculate_reconstruction_quality(original, reconstructed))
                    except Exception:
                        quality_score = 0.0

                    # 阈值（支持0~1或0~10配置）
                    threshold = getattr(self, "quality_threshold", None)
                    if isinstance(threshold, (int, float)):
                        threshold_score = threshold * 10.0 if threshold <= 1.0 else float(threshold)
                    else:
                        threshold_score = None

                    # 先执行基础校验
                    basic_ok = self._validate_reconstruction(original, reconstructed, language)
                    if not basic_ok:
                        logger.warning(f"变体 {i+1} 基础校验未通过，跳过")
                        self.augmentation_stats["failed_samples"] += 1
                        continue

                    # 再执行阈值过滤
                    if threshold_score is not None and quality_score < threshold_score:
                        logger.warning(f"变体 {i+1} 质量评分 {quality_score:.2f}/10 低于阈值 {threshold_score:.2f}/10，丢弃")
                        self.augmentation_stats["failed_samples"] += 1
                        continue
