...
class MemoryOptimizer:
    ...
    def __init__(self, memory_limit_mb: Optional[float] = None):
        # 动态默认：按系统内存的 70% 计算；如可用，优先使用平台适配建议
        if memory_limit_mb is None:
            try:
                from src.config.env_adapter import get_optimal_memory_limit  # 延迟导入避免循环依赖
                computed = int(get_optimal_memory_limit())
            except Exception:
                vm = psutil.virtual_memory()
                computed = int((vm.total / 1024 / 1024) * 0.70)
            self.memory_limit_mb = computed
        else:
            self.memory_limit_mb = int(memory_limit_mb)

        self.process = psutil.Process()
        self.model_cache = LRUCache(max_size=2, memory_limit_mb=self.memory_limit_mb * 0.7)
        self.data_cache = LRUCache(max_size=10, memory_limit_mb=self.memory_limit_mb * 0.2)
