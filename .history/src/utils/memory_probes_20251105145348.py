...
PROBE_CONFIG = {
    ...
    "log_level": "INFO",            # 日志级别
    "auto_inject": False,            # 禁用自动注入探针（避免导入不存在的模块）
    "injection_points": [           # 默认注入点配置（已禁用）
        # 注意：这些注入点已被禁用，因为某些模块可能不存在
        # 如需启用，请确保模块存在并修改 auto_inject 为 True
        # {
...
# 追加默认激进清理阈值，并从配置加载覆盖
PROBE_CONFIG.setdefault("aggressive_threshold", 0.85)

# 从 configs/quality_probes.json 加载 memory 段覆盖阈值/间隔
try:
    import json as _json
    from pathlib import Path as _Path
    _cfg_path = _Path("configs/quality_probes.json")
    if _cfg_path.exists():
        with open(_cfg_path, "r", encoding="utf-8") as _f:
            _cfg = _json.load(_f)
            _mem = _cfg.get("memory", {}) or {}
            for _k in ("threshold_warning", "threshold_critical", "check_interval", "gc_threshold", "aggressive_threshold"):
                if _k in _mem:
                    PROBE_CONFIG[_k] = _mem[_k]
            # 同步日志级别（可选）
            if "log_level" in _mem:
                PROBE_CONFIG["log_level"] = _mem["log_level"]
                logger.setLevel(getattr(logging, str(PROBE_CONFIG["log_level"]).upper(), logging.INFO))
except Exception as _e:
    logger.debug(f"加载内存探针配置失败: {_e}")
