...
# 兼容旧命名：部分旧模块仍引用 MediaProcessingError
# 为保持向后兼容，这里将其别名到新的 VideoProcessError
try:
    MediaProcessingError = VideoProcessError  # type: ignore[name-defined]
except Exception:
    # 在极少数导入顺序异常情况下，忽略别名失败
    pass


class DataLakeError(ClipMasterError):
    """数据湖相关错误（兼容 data.hit_pattern_lake 模块）"""
    def __init__(self, message="数据湖访问错误"):
        super().__init__(message, code=ErrorCode.GENERAL_ERROR, critical=False)
