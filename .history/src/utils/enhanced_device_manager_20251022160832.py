...
# 尝试导入nvidia-ml-py（推荐）或pynvml（已弃用但向后兼容）
try:
    import pynvml
    NVML_AVAILABLE = True
    # 检查是否是新版nvidia-ml-py
    if hasattr(pynvml, '__version__'):
        logger.info(f"使用nvidia-ml-py版本: {pynvml.__version__}")
    else:
        logger.warning("使用已弃用的pynvml，建议升级到nvidia-ml-py: pip install nvidia-ml-py")
except ImportError:
    NVML_AVAILABLE = False
    logger.warning("NVML不可用，GPU监控功能受限")
