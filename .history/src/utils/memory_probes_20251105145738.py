...
# 从 central configs/memory_config.json 加载覆盖
try:
    from src.utils.memory_config import get_memory_config
    _cfg = get_memory_config()
    for _k in ("threshold_warning", "threshold_critical", "check_interval", "gc_threshold", "aggressive_threshold", "log_level", "auto_inject"):
        if _k in _cfg:
            PROBE_CONFIG[_k] = _cfg[_k]
    # 同步日志级别
    logger.setLevel(getattr(logging, str(PROBE_CONFIG.get("log_level", "INFO")).upper(), logging.INFO))
