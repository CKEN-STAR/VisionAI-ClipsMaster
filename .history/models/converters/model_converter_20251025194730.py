...
class ModelConverter:
    ...
    def _convert_to_gguf(self, model_path: str, output_path: str, quant_type: str) -> str:
        ...
        # llama.cpp的convert_hf_to_gguf.py支持的outtype参数
        quant_mapping = {
            'Q4_K_M': 'q4_k_m',  # 4-bit K-quants (medium)
            'Q5_K': 'q5_k_m',    # 5-bit K-quants (medium)
        ...
        # 获取量化类型
        outtype = quant_mapping.get(quant_type, 'q8_0')

        # 使用sys.executable确保使用当前虚拟环境的Python
        cmd = [
            sys.executable,  # 使用当前Python解释器
            str(convert_script),
            str(model_path_obj),
            "--outfile", str(output_path_obj),
            "--outtype", outtype
        ]

        self.logger.info(f"Starting GGUF conversion:")
        self.logger.info(f"  Source: {model_path}")
        self.logger.info(f"  Output: {output_path}")
        self.logger.info(f"  Quantization: {quant_type} -> {outtype}")
        ...
        try:
            ...
            # 记录输出
            if result.stdout:
                self.logger.info(f"Conversion output:\n{result.stdout}")
            if result.stderr:
                self.logger.warning(f"Conversion warnings:\n{result.stderr}")

            # 验证输出文件
            if not output_path_obj.exists():
                raise FileNotFoundError(f"Conversion completed but output file not found: {output_path}")

            file_size = output_path_obj.stat().st_size
            self.logger.info(f"Successfully converted to GGUF: {output_path}")
            self.logger.info(f"Output file size: {file_size / 1024 / 1024:.2f} MB")

            return str(output_path_obj)

        except subprocess.CalledProcessError as e:
            error_msg = f"GGUF conversion failed with return code {e.returncode}"
            if e.stdout:
                error_msg += f"\nStdout:\n{e.stdout}"
            if e.stderr:
                error_msg += f"\nStderr:\n{e.stderr}"
