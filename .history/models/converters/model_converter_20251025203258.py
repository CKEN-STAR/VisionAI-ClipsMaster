...
class ModelConverter:
    ...
    def merge_lora_to_base(
        ...
        adapter_path_obj = Path(lora_adapter_path)
        ...
        if not (base_path_obj / "config.json").exists():
            raise FileNotFoundError(f"config.json not found in base model: {base_model_path}")
        if not (adapter_path_obj / "adapter_config.json").exists():
            raise FileNotFoundError(f"adapter_config.json not found in adapter: {lora_adapter_path}")
        ...
        self.logger.info(f"Merging LoRA adapter into base model:")
        self.logger.info(f"  Base model: {base_model_path}")
        self.logger.info(f"  LoRA adapter: {lora_adapter_path}")
        ...
        try:
            ...
            self.logger.info(f"✅ Successfully merged LoRA adapter")
            self.logger.info(f"   Output: {output_path}")

            return str(output_path)

        except Exception as e:
            self.logger.error(f"LoRA merge failed: {e}")
            raise
