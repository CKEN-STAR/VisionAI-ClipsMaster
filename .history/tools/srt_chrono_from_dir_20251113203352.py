...
def collect_items_from_dir(dir_path: str) -> List[Dict[str, Any]]:
    ...
    for root, _, files in os.walk(dir_path):
        for fn in files:
            ...
            # 仅保留合理的“集数”范围，过滤混剪/中间产物（通常带超长数字ID）
            if not (1 <= ep <= 300):
                continue
            fp = os.path.join(root, fn)
            # 排除带有 #ORIGINAL: 元信息的文件（一般为我们生成的混剪SRT）
            try:
...
def main():
    ...
    ap.add_argument('--retain', type=float, default=float(os.getenv('CHRONO_RETAIN', '0.7')), help='每集保留比例(0.3-0.9)')
