...
def main():
    ...
    sample_output = {
        ...
        "adapted_patterns": {
            "短视频": adapter.adapt_pattern(patterns[0], "短视频"),
            "互动剧": adapter.adapt_pattern(patterns[0], "互动剧"),
            "广播剧": adapter.adapt_pattern(patterns[0], "广播剧"),
            "漫画": adapter.adapt_pattern(patterns[0], "漫画")
        }
    }
    
    with open("cross_media_adaptation_examples.json", "w", encoding="utf-8") as f:
        json.dump(sample_output, f, ensure_ascii=False, indent=2)
    
    logger.info(f"示例已保存到文件 cross_media_adaptation_examples.json")
    logger.info("\n跨媒介模式迁移独立演示完成")
